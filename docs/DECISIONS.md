...
*Corrected on 2026-09-11, at 7.0(a), against the vendor.* "No minute bars exist for any night before capture begins, at any price" was true of the store and false of the vendor, which sells one-minute history back to 2004; the measurement is recorded under **No execution variant is admitted in this generation, and the condition that would reopen it is named**. **No ruling changes here.** 5.3 still screens selection variants and not execution ones, and the ground is the evidence store's: it holds no minute from before capture to replay a stop against, and a minute bought now could not stand as a past night's evidence.
...
**The trade chain refuses past its window rather than carrying a lateness stamp**
Ruled at 6.10, closing the row raised at the 5.7 sign-off. Every reader in the trade chain is pinned at `observed_at <= EndOfSession(sessionDate)`, being `TriggerResolutionReader.ForLiveSession` for `RiskGate`, `TradeOrderReader.ForLiveSession` for `PaperBroker` and the same shape in `TriggerResolver`, so a rerun after local midnight of the session's own day writes rows the next stage can never see. `RiskGate` then recorded `clean` with "no plan resting in this session was touched", which is a clean run over a read it could not make. The three stages now refuse past that instant.

**Two closing moves were available and the cheaper one is also the right one.** The other is the lateness stamp the decision above describes, which `recheck` and the sector walk carry. It is the right shape where the late answer is still worth something, and it is not here. The chain's rows are not answers the session asked a vendor for; they are the session's own decisions. A sector fetched six hours late is the same sector, and a trigger resolved a day late is not a late decision but a different one, taken with a day of hindsight about where the price went. Admitting it under a bound would be the improvement-after-the-fact the decision above exists to forbid, arriving through the mechanism built to permit a narrow case of it.

**So the exception stays one column wide, which is what that decision says of itself.** `setup.correction_lateness_minutes` is the whole of the late path and nothing in the trade chain joins it.

**The refusal is recorded rather than thrown.** A stage that threw would reach a console and nothing else, and a console is not something this lab keeps. It writes a failed run with the reason on it, so the night's row says the slot did not end cleanly, the morning screen puts it in the ragged bucket, and the operator reads why. A refusal nobody can see is the same silence with a different cause.

**It has cost nothing and no instance is claimed.** `trade_plan` holds nought rows and has on every night the lab has run, so the three slots the branch guard refused on the evening of 2026-09-04 had nothing to do. A mechanism capable of producing a fault is not evidence that it produced one, and this one is armed for the first night that has a plan. The case is authored for that reason.
...
**An authored fixture row's geometry is authoritative and its verdicts are computed from it**
Taken at 6.1 on 2026-09-06, closing the row raised at 5.3. The golden fixture holds one authored setup, `IESC-long`, inserted so `SignalVectorizer` has a subject on a night the detectors flagged nothing long. Its trigger and stop are authored; its check verdicts came from the shipped rules over the evidence the detector assembles; and `SignalVectorizer` froze the signals off the authored prices. So the row recorded verdicts measured against one geometry beside signals frozen from another, and `trigger-near` and `exit-tight` read as failures on a row whose frozen distances clear both thresholds. The acceptance test at 5.3 was the first thing to hold the two halves against each other and reported them as its two disagreements, which was correct behaviour and is why the figure was 2 rather than nought.

**Both repairs were priced and the assembled half was refused.** `IESC` split on the fixture's own session, so the raw highs behind it are on the pre-split basis and the pullback's real geometry gives a trigger of 800.999 against a close of 324.12; that is why plausible prices were authored in the first place. Writing the assembled geometry back makes the row coherent and makes its data absurd: a trade planned at more than twice the price the stock closed at, rendered on the gallery, read by every later session and re-derived by each of them before it can be dismissed. The row is authored on purpose, and judging an authored geometry by other geometry is the incoherence rather than the authoring.

**So the authored half is the row and everything else follows it, through the shipped arithmetic.** The two geometry gates are evaluated over evidence carrying the authored trigger and stop; the distances come from the same helper the detectors and the vectorizer compute them with, so nothing here re-implements a rule; and the row's stored give-up distance is derived from its own trigger and stop where it was a third authored literal agreeing with neither the prices above it nor the name's own range. The two hand-derived verdicts recorded at 2.6 move with it, and the derivation that produces them is restated over the geometry the row carries rather than over the bars (see: A fixture expectation changes only with a recorded reason, and the report counts what changed).

**What this does not license is authoring a verdict.** The verdicts are still the shipped rules' answer, run over evidence, and an authored row carrying an invented verdict would be a test of the test. What is authored is the input, which is the part a detector cannot supply for a name that has not pulled back (see: Gate boundaries are exercised by authored cases and the captured fixture is not asked to do it).
...
*Noted on 2026-09-11, at the 7.12 sign-off.* From 7.8 the stop is resolved at the entry minute, so the evening give-up distance is not the stop a generation 1 trade takes, and 7.0 retired it from selection as the mismatch (see: The give-up gate is retired at selection and reborn as the entry-time ceiling). The mechanism above is therefore generation 0's, and from the switch night the cap ranks generation 1's candidates on a quantity its own rule no longer uses, with a candidate that has none ranked after every one that has, which 7.11 added. **The ranking stays in force**, because a decision is changed only by another decision, and which key generation 1's cap ranks on is carried to the operator in the obligations row raised at 7.11 and widened at 7.12.
...
**The third is the gate, and it is what `plan_audit` was designed around.** RiskGate may reduce a size and may never recompute one, so what is compared here is an intention against an outcome rather than two runs of one formula (see: Order prices and the share count resolve at the entry minute). The cap that bound is on the row, so a reduction reads as a decision rather than as an unexplained difference. *Citation repointed on 2026-09-11, at 7.8*: it named the plan-size entry, which moved to Previously decided that day; on a plan carrying the rule the size compared is the one the entry minute resolved, and the gate still only reduces it.
...
*Corrected on 2026-09-03, at 5.2, on the row raised at 4.13.* The paragraph above names the setup's own session and the anchor is the trigger's. It was the setup's until 4.18, which counted from `trade_plan.as_of`, the evening the setup was flagged; 4.18 moved the count to the session the trigger was touched in, which is what its own row asked for and what the aftermath has been measured over since. **The two differ by exactly one session**, a plan being live in exactly one, so a later session implementing or verifying against the sentence as it stood would anchor the window a session early and every figure would be off by one bar in the same direction. **Nothing else in the paragraph moves**: the count is from the bars rather than from a calendar, the arithmetic is inclusive so eleven sessions is ten having passed, and a name whose series the fetch has not reached waits rather than becoming unclassified. **A correction rather than a new decision**, on the terms the order-price entry took one at 4.18: the mechanism moved and nothing the operator answered did.
...
*Clause (c) superseded on 2026-09-02, by the operator, on the measurement of 5.0(a).* The thrust is not the hit with the extreme; it is the most recent qualifying hit, then rank, which is what the detectors have always done, and the reasons are the next entry's. Clauses (a), (b) and (d) stand and are built.

*Clauses (a) and (b) superseded on 2026-09-11, at 7.8, by **Order prices and the share count resolve at the entry minute** and **The stop switches to the entry candle at more than 2%, and 5% as the switch is fourth-hand**.* The prices are no longer computable on the evening before, because the plan carries the rule and the entry minute resolves them, and the give-up point is the session's extreme as of that minute or the entry candle's rather than 0.1 ADR beyond the final pullback session's. Clause (d) stands and is built: a setup with no pullback gets no plan. The entry stays live for that clause and for the reasoning of the other three.

**The thrust is the most recent qualifying hit inside the window, then rank, and the extreme clause of the order-price decision does not ship**
Ruled by the operator on 2026-09-02 with the figures of 5.0(a) in front of them, closing question 6 of the phase 5 sitting, the one question the sitting did not ask. It supersedes the third clause of (see: The order prices are derived from the final pullback session's minutes, not from the screening geometry), which said that where several scan hits fall inside the window the thrust is the one with the extreme, ties broken by recency. The other three clauses of that decision are untouched.

**The rule, stated because it was written nowhere.** Among the scan hits inside the ten-session window on the setup's own side, being `gainer`, `gapper` and `leader` for a long and `decliner`, `gapdown` and `laggard` for a short, the thrust is the hit on the most recent session, and where two scans hit the same session the one with the better rank. The geometry runs from that hit over that scan's span, one session or twenty, and the anchored average price is anchored off the swing that hit's move ran from. `LongSetupDetector`, `ShortSetupDetector` and `SignalVectorizer` each read the hit that way and `CheckRecomputer` keys the cluster count on the hit they chose, and each of the four cites this entry at the line that does it.

**The first reason is the measurement, and it is a measurement over this store.** Over the 602 calibration sessions the extreme rule moves the trigger and the stop on 3,142 long rows and 1,610 short rows, and not one of them is a candidate: the 24 long survivors are the same 24 rows under both rules, with the same trigger and the same stop on every one, and short survives nought under both. So the level the clause was written to correct moves on about one row in ten and on no row that would have traded. Long and short are stated apart and the figures are the 5.0(a) entry's, over calibration rows reconstructed against today's membership (see: Long and short are never pooled into one figure).

**The second reason is a property of the rule and not of this store, and it would decide the question alone.** The peak session of a twenty-session run is often a one-session hit in its own right, and the literal extreme prefers that hit because its own high is the highest. The extreme is the same bar either way; what changes is the origin, and a one-session span measures the give-back against the peak day alone where the twenty-session span measures it against the whole run. The same dip is a third of one and most of the other, and `dip-shape` fell from 3,182 to 2,952 on that alone. Any store with one-session and twenty-session scans over the same names has this property, because it is a fact about what "extreme" selects and not about which names were flagged.

**The span-reaching reading is not measured and not adopted.** Under it the hit whose span reaches the extreme would win and ties would go to the later hit, which is nearer the shipped rule than the literal one. It could only move rows no gate passes: the survivor set has been shown insensitive to which thrust is chosen, and a reading that cannot change a candidate is not worth a calibration run. It is named so a later session does not find it and measure it as if it had been overlooked.

**What this leaves in place and what it unblocks.** The calibration figures since 2.11 are the figures of the rule that stays, so the repair pile at 5.8 repairs figures that will not move again, and V0's detection at 5.1 is the rule that has run every night. The once-only threshold adjustment is untouched by this and stays unspent.
...
**A version whose moved gate cannot be judged from the frozen signals is refused at admission**
Settled at 5.2 on 2026-09-03, on the reasoning the operator already applied to the execution family. A selection version is scored by replaying its one moved gate over the signals each night froze. A gate whose threshold compares one frozen signal replays as a lookup; a gate whose quantity is arithmetic over several does not, and computing that arithmetic in the replay would be a second implementation of a step the detector already takes, which is what the acceptance test at 5.3 exists to refuse (see: A selection rule is the gate list plus a named threshold per gate, and one implementation reads it for the detector and the harness alike).

**Two gates, both on the short side, and they are named rather than discovered.** `averages-squeezing` compares the 21-to-50 gap against its own twenty-session average, which is a ratio of two frozen signals; `reached-ceiling` compares the nearer of two average distances over the daily range, which is three. Every other movable threshold on either side names one signal. So the long side's ten movable thresholds are all admissible and the short side's twelve are ten, and the two counts are stated apart (see: Long and short are never pooled into one figure).

**Refused rather than admitted and left open**, on exactly the grounds that close the execution family: a version nothing can score stays open with its age on the ledger for ever, because no timeout quietly closes one (see: Targets and minimum samples are written at creation and are immutable) (see: Generation 1 opens with no version admissible in either family, and what reopens each is named).

**What would lift it is named and costs no vendor call.** Freezing the compared quantity as a signal of its own makes the gate a lookup like the rest. That widens the signal library rather than the rule shape, so it is the channel this loop already has rather than a structural change (see: Generation 1 opens with no version admissible in either family, and what reopens each is named).

**Lifted at 6.1 on 2026-09-06, by exactly that.** `ema_gap_21_50_over_avg` and `ceiling_distance_ranges` are the two quantities, each computed through one implementation in Core that the detector, the rule and the vectorizer all read, and SignalBackfiller put both onto every setup recorded before the change. The short side's movable thresholds are twelve of twelve and its judgeable gates nine of ten, the one it still loses being `downtrend`, which compares a grade and carries no threshold; the long side is unchanged at ten of ten and nine of ten, and the two are still stated apart. **The refusal itself stays and is now about nothing**, because it guards a property of the stored signals rather than of today's library: a signal added later with a derived quantity nobody froze would put a gate back where these two were, and a guard removed the day it stops firing is a guard that was never permanent. It is proved over an authored baseline rather than over the instance that happened to exist.

**A backfilled signal is read by a replay and not by a surface, because point-in-time is a property of the inputs**
Taken at 6.1 on 2026-09-06, when SignalBackfiller first wrote one. A backfilled value is computed long after the night it describes, from data bounded at that night: the stage passes each setup's own session as the as-of, so every read behind it is bounded where the night's read was bounded and only the arithmetic is late. Two questions can be asked of such a row and they have different answers, so there are two reads and each is named for its question.

**What did the night's decision rest on** bounds the instant the value was frozen. A signal computed months later is not something the detector could have compared against, so a screen showing one as the night's own evidence would claim a comparison nobody made. That is `SetupSignalReader.Read`, and every surface uses it.

**What may the lab now compute about that night** does not bound it, because the value is a function of the night's own inputs. That is `ReadIncludingBackfilled`, and the replay harness and the scorer use it. Without it a backfilled signal would be visible to nothing at all and the backfill would enrich a store nobody could read, which is the whole of what makes widening the library cheap: a signal admitted today is computable for every night the lab ever recorded.

**Backdating the row was the other way to close it and it is refused.** Stamping a backfilled value with the session's own end of day makes one read serve both questions and makes the store lie about when the arithmetic ran; it is the same act the forward fill names as the one thing that would break its own property. A second read costs a method and keeps both answers true.

**The refusal is a real one and the wrong caller would be a defect nothing else could see.** Both reads return the same shape over the same table, and a surface pointed at the wider one would render plausible numbers under a heading that says the night compared against them.

**A figure a seeded algorithm produces is not independently derived, and the tier says so**
Settled at 5.2 on 2026-09-03, closing the row raised at 3.5. `DERIVED` means a second method sharing no line of code with the engine. A restatement of a seeded resampling scheme shares no line and shares everything else: the generator, the seed, the block order and the tail convention are all required for the two to agree at all, so agreement establishes that neither was mistyped and nothing more. The tier read as independent verification over figures that hold change detection, which is what `FROZEN` already means.
...
**A selection version's target is derived from the settling rule and is not typed**
Taken at 6.7, which is the first checkpoint with a component that has to act on a target. `variant.target` is prose an operator gives at registration, and the gate that settles against it cannot read a word of it. 5.1 made exactly this repair for `definition` and stopped short of the column beside it: a selection version's definition is derived from the threshold it moves and may not be typed, because a sentence somebody types can disagree with the columns beside it and the day it does there is nothing to say which of the two the version is.

**It is worse for the target than for the definition, because the target is the thing pre-registration exists to fix.** A definition that disagrees with its columns describes the version wrongly. A target that disagrees with the rule the gate runs means the version was settled against something nobody wrote down, and no reading of the register afterwards can tell which sentence was in force. Twenty worthless candidates give a 64% chance that one looks impressive by luck, and the defence is a target that cannot move; a target the gate ignores has already moved.

**So the rule lives in one place and the column is written from it.** `AcceptanceTest.Describe` states the quantity, the interval, the bound that accepts, the diagnostic that rejects on its own and the sample the question may not be asked before, and the admitter refuses `--target` for a selection version on the terms it already refuses `--definition`. What is on the ledger is then the rule that will run.

**The baseline keeps a typed target and that is not an exception to this.** It is the arm every other version is differenced against, so there is no baseline-minus-baseline series and this gate never reads it: V0's own pre-registration says so in terms, that it is not itself accepted or rejected and closes only if the baseline is edited, which starts a new generation. A derived target there would describe a settlement that cannot happen.

**Pre-registration is immutable, so V0's typed target stands.** It was written on 2026-09-03 and says what the baseline is; nothing here rewrites it, and the rule applies to the versions registered after it (see: Targets and minimum samples are written at creation and are immutable).

**A version's night is one paired observation, and the minimum it is read against was derived over a different population**
Taken at 6.7, and it is a finding as much as a decision. `variant_score` is a difference of two means over two overlapping selections, so there is no per-name pairing to take and no within-night dispersion to record, which migration 052 says in its own words: what is paired is the night. `PairedInterval.Disperse` reads a series that cannot say how its own pairs dispersed as one observation a night, on its own stated principle that an unknown is read the safe way rather than the flattering one. So a version's effective count rises at most one a night.

**The minimum it is read against is 1,802 effective paired setup observations, and that figure was derived at 53.5 pairs a night over band 1's population.** The derivation of 5.0(b) simulated series shaped like the store's, reached 90% power at 202 nights, and counted what those series carried; a version's series carries one observation a night over the same calendar. So the pre-registered figure means about 202 nights where it was derived and about 1,802 nights here, which is seven years, and the difference is a property of the unit rather than of the power calculation (see: The minimum sample is 1802 effective observations, derived against the interval actually run over the flagged population's dispersion).

**Nothing here changes the figure and nothing here works around it.** The gate counts honestly, states the population on every reading, and leaves a version short of its minimum open with the shortfall in words. A re-derivation over the version population is the operator's ruling and a build session's derivation, on exactly the shape the minimum sample itself took, and it is a row at the operator rather than an act this checkpoint takes: ARCHITECTURE already says the figures are defaults overridable per proposal with a stated reason, so the repair is a second derivation and not an edit to the first.

**A night the two rules selected identically is not an observation of the version, and that is the half that could have gone wrong quietly.** Such a night carries a difference of exactly nought by construction, so counting it would advance the sample toward maturity while pulling the mean toward nought with nights the version was never exercised on. Every count would have been right and the sentence beside them false, which is the shape this corpus keeps finding. Those nights are counted on the reading and excluded from the series.
...
*Noted on 2026-09-11, at the 7.12 sign-off.* One running version's execution changes in place, and the reason above does not reach it. From the night the production checkout first carries 7.8 until the switch night, V0's plans execute generation 1's entry rule and, from 7.10, its exits, while V0 is still generation 0's open baseline (see: Order prices and the share count resolve at the entry minute) (see: Generation 1 trims 15% at 3R and again at 5R on both sides, and a short is held three sessions rather than trailed). **Both halves of the reason are false of this case.** It is detectable afterwards, row by row: migration 067 gives every plan an `entry_rule` of `evening-prices` or `flush-reclaim`, with a constraint holding its prices present on exactly the first, and every exit records the rule that produced it. And nothing exists for it to contaminate: V0 holds no trade and no score and has no open version beside it. **The window is the operator's**, opening when the production checkout moves past 7.8 and closing when generation 1 is registered, and it is as long as the gap the operator leaves between the two. The ruling stands as written.

**Generation 0 is retired as measuring the entry-level mismatch, and generation 1 registers only once its rule is whole**
Decided on 2026-09-11 at 7.0, on the operator's instruction of 2026-09-10 to realign the lab to the strategy it models. Generation 0's gate set was authored from interviews nobody wrote down, and its binding gate compares a daily-bar span against a threshold written for an intraday entry level, so every funnel figure since 2.11 measures that mismatch rather than the pattern. `SOURCES.md` traced all twenty clauses against the material, and generation 1 is the gate set and the execution rule that trace supports.

**The freeze rule is not touched and is what makes the order matter.** Editing a registered baseline closes every open version as unresolved and starts a new generation, so a baseline registered before its rule is whole would be frozen on the old execution and the next change would spend another generation. So generation 1 registers once, at 7.11, naming both families: the selection gate set and the execution rule, being the level set, the break and reclaim, the two-part stop with its switch and ceiling, the trims and the trail. A version of generation 1 differs from that baseline in one family and never in both. The gate set is written and its detector built and proved against the fixture long before that, and nothing registers until the rule it belongs to exists and has been measured.

**What closing generation 0 costs is nothing that exists.** The register holds V0 alone, generation 0, open; no score exists, no trade has fired and no holdout window has been spent. Generation 0's detector keeps recording live until the switch and then beside generation 1 for comparison, because a night not recorded is gone for good.

**It will supersede two entries at 7.11, in the commit that performs the switch,** because shipped source cites both until then and moving them earlier would leave `no-superseded-citation` failing or the corpus contradicting the running code. They are **A version changes one threshold over the existing gate list, and structural change is out of scope for this generation** and **No execution variant is admitted in this generation, and the condition that would reopen it is named**. Both are scoped to generation 0 in their own headings, and after the switch "this generation" would be read as generation 1, which neither describes.

*Landed on 2026-09-11, at 7.11.* The detectors read the generation in force from the register, so from the night generation 1's baseline is registered through the act that closes a generation they write generation 1's verdicts to `setup` and generation 0's to `setup_generation_zero`, and the check register retires generation 0's clauses that night. Both supersessions moved with the code, into **Generation 1 opens with no version admissible in either family, and what reopens each is named**.

**A one-time backfill is outside the nightly ceiling, whether it buys daily history or minutes**
The ruling of 2026-08-25 lives in `RUNBOOK.md`: "The ceiling guards the evening's job, and a one-time operation is not the evening's job." It named the daily-bar backfill because that was the backfill in front of it, and its reasoning is about what kind of operation it is rather than about its size. So it was too narrow rather than wrong. **A one-time minute backfill is a one-time operation and not the evening's job, on the same reasoning**, with the daily-bar backfill as one instance. The run records its calls like any other, and `counts_against_ceiling` already says whether the daily total sees them.

**Charging it against the ceiling was refused**, because the nightly would compete with it for every day it ran, and a night not recorded is gone for good. The probe at 7.0(a) sizes a fetch and sets how many days it spreads over; it does not decide which budget the fetch belongs to, which is this entry.

**The baseline is written clause by clause from SOURCES.md, and a figure he states screens even where he qualifies it**
Decided by the operator on 2026-09-15 at 7.19, after generation 1's first night, and in force from generation 2. Three cases, and each has one treatment. Where the source gives a form and a number, the baseline ships both, quoted. Where it gives a form and no number, it ships the form with the number marked as the author's and carried in the limits table, where the operator may replace it without reopening anything, and the once-only threshold adjustment carries over unspent (see: The once-only threshold adjustment is recorded unspent, and the baseline is frozen without it). **Where the source gives a figure and qualifies it, the figure screens.** `moves-enough` is the instance: "I mainly trade stocks with >5% ADR for this reason, but this isn't a hard rule. I sometimes trade slower stocks with a tighter stop, depending on what's working lately and the quality of the setup" (`P3`).

**Generation 1 read the qualifier the other way, and its first night is why the reading is reversed.** Recorded and never required, the daily range admitted every name the other clauses passed, and on 2026-09-14 18 of the 24 long names generation 1 passed moved less than 5% a day. The slowest moved 0.3%, a stock sitting flat after one jump, which the ladder, the thrust, the dip and the contraction all read as a pullback. What his qualifier describes is an exception he takes on a judgement of the setup and of what is working, and a rule that cannot take the judgement is left with the figure he trades by rather than with no figure. **The cost is stated rather than hidden**: the lab now refuses the slower names he would have taken along with the ones he would have refused, and it cannot tell the two apart.

**The recording floor does not move**, so a slower name is still recorded, failing the clause, and generation 1's verdict on any night after the switch is a reading of the stored vector rather than a night nobody recorded.

**What this cannot answer is a missing form or an uncomputable one**, and those are the operator's rather than the author's: the anchored average price's anchor, which he tunes to the candle the price has already respected and which cannot be computed at 18:30 the evening before, and the re-entry rule, for which no source gives a form at all. Both are carried as obligations due at the operator from 7.0.

**The give-up gate is retired at selection and reborn as the entry-time ceiling**
`exit-tight` compares a distance measured from a daily-bar extreme against a cap written for an intraday entry level, so at selection it measures the mismatch the whole phase exists to remove. **In generation 1 it screens nothing at detection.** Its sourced form is the stop-too-wide test at the minute of entry, which is where his own words put it, so it moves there and applies to the stop the entry actually resolves (see: Order prices and the share count resolve at the entry minute).

**It supersedes part of one entry at 7.8 rather than now.** The entry is **Two of the six limits are not applied at trigger, and which two is stated rather than left to the code**, which places the give-up cap at detection; shipped source cites it until 7.8's code lands, and the supersession moves with that code.

*Landed on 2026-09-11, at 7.8.* The entry moved to Previously decided with the code, superseded by this one and by the entry-minute decision together, since risk per trade moved in the same commit.

**The entry ceiling is the tighter of half the daily range and 5%**
His three statements of it share one structure, and the structure is the rule. "I limit my stops at less than 50% of these stock's average [ADR], so maximum will be 5%" (`P1` 17:48); "avoided exceeding half of the stock's average daily range (ADR), so, no more than a 2.5% stop for a 5% ADR stock" (`P3`); "limit my stop at less than 50% of the stock's ADR... So the maximum stop I would take is around 5%" (`P2` 25:48). **The half-range limit is the rule and 5% is what it comes to at the widest range he trades**, so the absolute figure binds only above a 10% range; on the 5% range floor his ceiling is 2.5%, and a lab refusing at 5% alone would take stops twice his width over most of the population.

**So the refusal at the entry minute is the tighter of the two, both cited, and the same on both sides**, because none of his three statements names a direction. The 2.5% to 3% he says he rarely exceeds is recorded beside it as a working maximum and screens nothing. This is a form with a number, so under the decision on what generation 1 is written from, nothing in it is the author's. It lands with the code at 7.8.

**The stop switches to the entry candle at more than 2%, and 5% as the switch is fourth-hand**
The base level is the session's extreme as of the entry minute, and where the stop it gives is more than 2% wide the stop moves to the entry candle's extreme instead: the low on a long, the high on a short. Both halves are his own words, the switch in a live example where he calls a day-low stop too wide at "more than 2%" (`P1` 2:12:13). **5% as the switch condition is a figure from social summaries that fused his absolute ceiling with this switch**, and `SOURCES.md` records it as fourth-hand so it does not enter the corpus as a citation on its way to becoming a threshold.

**It supersedes one clause at 7.8 rather than now.** The clause is (b) of **The order prices are derived from the final pullback session's minutes, not from the screening geometry**, being the give-up point placed 0.1 ADR beyond the final pullback session's extreme; the supersession moves with the code that replaces it.

*Landed on 2026-09-11, at 7.8.* The clause is marked superseded inside that entry, which stays live for its clause (d).

**Risk per trade stays at 0.75%, inside the strategy's own 0.5 to 1%**
The authored-parameters row's basis, "Midpoint of the 0.5 to 1% range the strategy is described with", is quoting his own written sentence rather than describing him: "And I keep account risk at 0.5 to 1% per trade" (`P3`). So the row carries a citation and no change. **His later statements put the figure at 0.5%**, twice as a limit in one passage (`P1` 15:44 and 17:48), and that is recorded rather than acted on: the value is his own range's midpoint, R is taken over the give-up distance and not over the risk fraction, so no figure the lab scores on moves with it, and moving it would re-size every version's book at once with no rule saying what that does to a paired comparison.

**Order prices and the share count resolve at the entry minute**
The entry the strategy states is a flush into a named level and a reclaim through the previous bar's extreme on a 1, 5 or 15 minute chart depending on how far into the session it is, so the price it fills at and the stop that follows from it are known at the minute it happens and not the evening before. **The plan carries the rule; the price, the stop and the size resolve at the entry minute.** One give-up price per trade still, resolved once, so the three rulings that there is exactly one give-up price stand and are cited as standing. RiskGate enforces risk per trade against the resolved distance rather than asserting a figure written the evening before.
...
*Landed on 2026-09-11, at 7.8.* Both supersessions moved with the code that replaces them, and every citation of the second in shipped source now names this entry.

**Generation 1's entry is armed by the flush into the hourly zone and taken on the first break after it, and a session gets one such decision**
Decided on 2026-09-11 at 7.8, reading the one sentence that states the mechanic, "a really, really, really fast flush to the downside into the support level, which is on the hourly 9 and the 21 EMA, and then I will look at the 1 minute or the 5 minute or the 15 minute candle for the first breakout of the previous bar high" (`P1` 1:56:39), with the short side its mirror on his word that he flips the long structure (`P1` 2:06:31). Six readings the sentence does not settle by itself, each one a later session could reasonably take otherwise with nothing in a count to show it.

**The zone is the two hourly averages and the flush is reaching its nearer edge**: the higher of the two for a long, the lower for a short. "Fast" names no measure and is not tested, so a slow drift into the zone arms the rule as a flush does, which is the author's reading and the one most likely to be revisited. **The break is a touch of the previous candle's extreme on the width in force**, through the same touch predicate every resting order in this lab uses, and the previous candle has to be the flush's own or a later one, so the rule cannot enter on a candle the flush had not reached. **The widths are one minute for the first fifteen minutes, five until sixty, and fifteen after**; the fifteen and the three widths are his, and the sixty is the author's, since he puts the second boundary at "maybe the 60 minute" (`P1` 1:57:14).

**The hourly averages are taken over closes that count the session's closing half hour as a bar.** The level he reads is a charting platform's hourly series, and those close the session's last bar at 16:00 as a bar, so a series that skipped it would be a different series from the one his eye is on. The grid's other ruling is untouched: the stub still cannot fire a rule that turns on an hourly close (see: The hourly grid anchors to the session open, and the closing stub is not an hourly bar). **A level needs sixty-three hourly closes behind it**, three times the longer period, the lab's own convergence rule for an exponential average; a name with fewer is unresolvable with the reason rather than read on a seed.

**The first break after the flush is the session's one decision.** Where the chase filter, the ceiling or the size refuses it, the plan is refused for the session rather than watched for a later break, because every refusal is of that entry and a watch that kept trying would be a second rule nobody described. **The stop is set through the entry minute's own bar**, its extreme included: the minute is the finest resolution the vendor sells and nothing says whether the extreme came before the break or after it, so the wider reading is taken, on the stance the fill model takes wherever one minute holds two events.

**The anchored average price is not one of the levels yet.** Its anchor rule is carried to the operator from 7.0 and cannot be computed at the entry minute as he describes it, so the zone is the two hourly averages alone, and 7.9's report says so rather than measuring over a level set it did not have.
...
**Generation 1 opens with no version admissible in either family, and what reopens each is named**
Decided on 2026-09-11 at 7.11, in the commit that performs the switch, carrying the two rulings that were scoped to generation 0 in their own headings into generation 1 in words that say which generation they are about.

**A version still differs from its baseline by one gate's threshold, over its own generation's gate list, and structural change is still out of scope.** What decided that for generation 0 decides it for generation 1: the harness's acceptance test is meaningful over a threshold change and vacuous over a structural one, and "differs by exactly one clause" is mechanical only over a gate list with named thresholds (see: A selection rule is the gate list plus a named threshold per gate, and one implementation reads it for the detector and the harness alike).

**What is new is that generation 1 has no such rule written down.** `SelectionRule` holds generation 0's gates and thresholds, and generation 1's detector evaluates its clauses from the evidence directly. So a threshold named after the switch would move a rule the lab no longer runs, and a version admitted over it would be differenced against a baseline it does not describe. `VariantAdmitter` refuses a selection version while generation 1 is in force, the pack refuses to cut rather than ask the researcher for a move over generation 0's rule, and the harness replays generation 0's rule over generation 0's rows alone. **What reopens it**: generation 1's gate lists written as a selection rule, with the harness reproducing generation 1's own selections over the nights it recorded.

**No execution version is admitted either, and its reopening condition moves.** It cannot be screened, because nothing replays an execution rule over the calibration minutes the backfill buys, though for the first time there is a history for one to run over; and it cannot accumulate, because no trade has fired. **What reopens it**: an execution replay over those minutes, or a funnel that produces trades.

**Refusing both costs nothing that exists.** Generation 0's register held its baseline alone when it closed, and generation 1 opens the same way: the baseline recording and planning every night, generation 0's companion beside it for comparison, and nothing in the register that cannot be scored (see: Generation 0 is retired as measuring the entry-level mismatch, and generation 1 registers only once its rule is whole).

It supersedes **No execution variant is admitted in this generation, and the condition that would reopen it is named** and **A version changes one threshold over the existing gate list, and structural change is out of scope for this generation**.
...
**The seat runs on the subscription against claude-opus-5, and the API path stays live for the day the subscription stops**
Ruled by the operator on 2026-09-07, closing question 1 of the phase 6 sitting. **The transport is the subscription through the Agent SDK and the model is `claude-opus-5`.** The API implementation is built and tested alongside it and is not a fallback written later: switching is a configuration value and a key in `appsettings.Secrets.json`, which is what the narrow interface was bought for (see: The researcher transport is a configuration switch between subscription and API key, over a deliberately narrow interface).

**The switch is neutral only if the served-model string is unchanged, and that is why the provision is worth having rather than a formality.** A transport switch part way through a record forks it exactly as a model change does whenever the served string differs, and every proposal records the configured model, the transport and the served string, so the fork point is visible in the record rather than inferred from a date. Planning the switch does not make it free; it makes it legible.

**Cost decided nothing and is recorded so a later session does not reopen it on those grounds.** The API path at this model is $4.13 a year over 52 runs, and the whole spread across the published range is about eight dollars. What decided it is that the subscription costs nothing marginal against a plan that already exists and the API path is one configuration value away.

**The price of this path is an obligation carried for as long as it is live**, being the 1.5 obligation: the empty tool set is a property of configuration rather than of there being no mechanism, so the test asserting it is the whole of the guard on the rule that the AI writes only to the proposal store (see: The AI writes only to the proposal store). On the API path that property is structural and the obligation would be discharged rather than carried. **That asymmetry is the cost of the choice and is recorded here rather than rediscovered at the switch.**

**A second thing can force the switch and it is not the operator.** The terms governing Agent SDK use against a plan moved once in the three months before this ruling, a change being announced for 15 June 2026 and then paused, and no published statement establishes that an unattended scheduled job is within a plan's terms one way or the other. What is established is that individual automation is contemplated and that shared production automation is directed to an API key. So the provision covers a terms change as well as a cancellation, and the record is bounded by whichever arrives first, on the same footing as a vendor retiring a pinned model.

**An unavailable seat queues and returns nothing, and it is shown on the screen the morning it happens.** A lapsed subscription and a plan limit are the same shape as an exhausted budget: never a partial proposal, and never a night that reads clean over an ask it could not make. **The status band names the transport that was refused and why**, because a seat that cannot ask is a fact about the running lab rather than about the build, and nothing in the verification harness reaches the running lab (see: Every phase ends in a generated phase report, not in a page somebody looks at). Recorded and not shown would mean the operator learns of it a quarter later from a gap in the proposal record, which is the one failure this provision exists to prevent.

**A local stopgap seat is recorded, and it is excluded from the pack-version hit rate**
Ruled by the operator on 2026-09-07, alongside the decision above. **A local model on an OpenAI-compatible endpoint is a third implementation behind the same narrow interface**, selected by configuration, for the weeks the subscription is unexpectedly unavailable and the operator turns it on rather than losing the week. The interface does not change: a pack in, proposal text out.

**A proposal it produces is kept and does not count toward the success criterion, and that exclusion is the whole of what makes the stopgap safe.** The criterion compares proposals made against one pack version with proposals made against another, so it holds the reader fixed and varies the pack. A proposal from a different reader measures the reader. **Mixing readers inside one version's record is worse than forking it, because a fork is visible in the record and a mixture is not** (see: The evidence pack is versioned, and the success criterion is proposal hit rate by pack version).

**The exclusion rests on what the proposal row already carries**, being the transport, the configured model and the served string, so a stopgap proposal is identifiable without a further column and the hit-rate reading filters on the pinned seat rather than on a flag somebody remembers to set.

**Two things a stopgap proposal does not do.** **It does not fail a pack version by citing the planted null.** The tripwire's logic is that a competent reader citing a meaningless signal indicts the pack; a weaker reader citing it indicts itself, and the rule as written cannot tell the two apart (see: One meaningless signal is planted in the conditional tables). A stopgap proposal citing the null is discarded with the reason recorded and the version is untouched. **And it does not spend a holdout window**, on the same grounds: a window spent against a reader the record does not compare is a window gone for nothing (see: Holdout windows are quarters of forward-collected evidence, allocated as they mature, capped at eight).

**A stopgap records what actually pins it**, being the digest of the weights file, the quantisation, the runtime version and the sampling parameters. The endpoint's own model field is a label the operator configured and attests nothing about what ran; a re-quantisation or a swapped file is otherwise a change no record can see.

**And what makes those recordings evidence rather than assertions is that a local seat can be replayed.** Measured on 2026-09-07 against `qwen/qwen3.5-9b` at temperature 0: three runs over one pack, at two context sizes, produced identical output down to the completion token count. A digest that reproduces the recorded proposal is a check anybody can run, where a hosted served-model string can only ever be taken on trust because no hosted call can be re-derived. **So the local path is the strongest of the three on reproducibility and the weakest on competence**, and those are different axes rather than one.

**It was tested before it was permitted, and the figures are the reason the permission is conditional.** The pack over a filled store is **23,545 tokens**, not the 8,400 the model budget assumes. At an 8,192 context the endpoint refuses it outright; at 32,768 the answer fits with 8 percent to spare; at 50,176 it fits with 40 percent. **A stopgap seat is configured with a context that leaves room for the pack to grow**, the pack costing about 570 tokens per declared signal, or it will begin refusing the week the library grows and the refusal will look like an outage.
...
**Admission compares outcome-similar pairs against all pairs, and a population too small for the comparison is undecided rather than rejected**
Settled at 6.2, because the decision above says what admission is for and nothing said what it computes, and a criterion stated only as "tightens outcome-similar neighbourhoods" is one every implementation would read differently. **The statistic is the mean distance among outcome-similar pairs over the mean distance among all pairs**, taken in the z-scored space of the signals being compared, measured once without the candidate and once with it. A candidate is admitted where adding it lowers that ratio, and below one means outcome-similar setups sit closer than a typical pair, which is what the library is for.

**A ratio rather than a distance, and that is most of the decision.** Euclidean distance in a wider space is larger whatever the extra dimension holds, so comparing the admitted set against the admitted set plus one candidate on raw distance would refuse a perfectly discriminating candidate for exactly the reason it refuses a noise one, and both refusals would look like a working test. Dividing by the all-pairs mean removes the part of that growth which is arithmetic rather than evidence, because both means are taken over the same space.

**It does not make the figure blind to the width, and it should not.** An independent column adds the same variance to a near pair as to a far one, so it lifts the near pair proportionally more and the ratio moves toward one. A column carrying nothing about the outcome makes the space genuinely worse at separating outcomes, and a criterion built to ignore the width would have admitted it. **This was written the other way round first and a test disproved it**, which is recorded here rather than quietly corrected: the claim was that the width cancels, the assertion that it cancels failed on a column of noise, and what the ratio actually buys is the separation between a column that grows the distance and earns it and a column that grows the distance and does not.

**Outcome-similar is the median split, so the threshold carries no parameter of its own.** A pair is outcome-similar when its outcomes differ by less than the median pairwise outcome difference over the same population. Derived from the population rather than chosen for it, which is what stops a number being moved until a candidate passes.

**Correlation is asked first and in absolute value.** A candidate above the limit is refused whatever it does to a neighbourhood, because it is a signal already present wearing a different name, and admitting it would let one quantity be screened twice and pay the correction threshold once (see: The correction threshold scales with signals screened, not signals shown). Absolute, because a candidate correlating minus 0.95 with an admitted signal carries exactly as little new information as one correlating plus 0.95; a limit read as signed would admit every inverted restatement of the library. It is measured against each admitted signal rather than against an average of them, since an average would hide a perfect duplicate behind thirty unrelated columns.

**The judgement is taken per side and the two are never one figure.** The outcome is signed by direction, so a long setup and a short setup are not near each other in any sense the statistic means, and a pooled population would measure the gap between the two books and report it as discrimination (see: Long and short are never pooled into one figure). A signal earns its place if it tightens on either side, because the library is one library and a signal admitted for shorts is computed on every setup; it is refused only where both sides refuse it.

**No verdict is taken over fewer than 20 setups**, counting the rows carrying a closed outcome and a value for every signal compared. Both statistics are means over pairs, and a mean over a handful of pairs moves further on one row than any real discrimination would move it, so a verdict there would be a verdict about the sample. Below the floor the candidate is recorded **undecided with the reason**, which is a recorded outcome rather than a silence: the lab is under this floor today and will be for months, and a record that wrote nothing would be indistinguishable in six months from a library whose candidates were all measured and all failed.

**Four outcomes and not two, and the fourth is the one that matters most today.** Admitted and rejected are what the library ends up carrying. A candidate measured and found not to tighten stays a candidate, because nothing about it was refused and it can be asked again over a wider population. A candidate that could not be judged was never measured, and the two shapes of that are kept apart too: nothing computes it, which a build task closes, and the population is too thin, which only accumulation closes.
...
**The realised false-discovery bar is a reading of a pack and the version carries the procedure**
Settled at 6.4, because two sentences of the decision above are both right and cannot both be read literally. It says a pack version is the tuple of the section list, the signals screened, **the correction form and its computed thresholds**, and the model identifier; it also says **what is deliberately not in the tuple is the store's contents**. The family-wise threshold satisfies both, being the level divided by the number of signals screened and so determined by the version's own inputs. The false-discovery bar does not: the Benjamini-Hochberg step-up finds the largest rank whose p-value clears its own bar, so the number it yields is a function of the p-values, which are a reading of the store on one night.

**So the tuple carries the form, the level, the screened set and the family-wise threshold, and the realised bar is reported in the pack body.** Taken the other way, the version would fork every time the evidence moved, every version would hold one proposal, and the success criterion would be a comparison of populations of one (see: The evidence pack is versioned, and the success criterion is proposal hit rate by pack version).

**The wrong choice here is invisible, which is why it is a decision and not a note.** A packer that hashed the realised bar into the fingerprint would work, would be byte-stable, would pass every check this corpus has, and would quietly produce a new version most nights. The damage would show up only as a hit-rate table with one row per version and nothing to compare, months later, with no way to tell when it started.

**What a pack still states is both thresholds**, and that is unchanged (see: Admission is on the false-discovery rate and the family-wise threshold is recorded beside it). The multiple-comparison section reports the form, the level, the count screened, the family-wise threshold and the realised false-discovery bar together; only the last of the five stays out of the identity. Where no claim has been measured there is no realised bar, and the section says so rather than printing the level, because a level printed as a bar reads as a threshold in force.

**The rule in force is the pack's first section, because a proposal moves a threshold from a value**
Settled at 6.5, and found by asking a model rather than by reading the document. A proposal is the direction, the gate, the threshold name, the value it moves from and the value it moves to. The first four are nameable from the signal library and the fifth was in no section of the pack: `Variant history` is every past proposal and excludes the baseline by design, so the one thing the pack never showed was the rule actually running (see: A proposal is a document whose one change is the five fields the version register already stores).

**Given a filled pack on 2026-09-07 a model abstained twice with exactly that reason**, that the pack does not specify the current threshold values or gate definitions. It was right, no check in this corpus would have said so, and every check was green on the tree that produced the pack. That is the shape of a defect nothing here can catch: the document described a pack that was built correctly, and what was missing was not a claim anybody had made.

**It is the first section rather than a later one**, because the evidence after it is evidence about it, and a reader given the deciles before the rule has to hold the numbers until it arrives. **It enters the version tuple by construction**, the tuple carrying the section list, so the pack that states a rule and the pack that did not are different versions and every proposal stays attributable to what its reader saw (see: A pack version pins what the model saw, and byte-stability is what makes that claim checkable).

**It is read from the register and applied to the baseline rather than printed from the constants.** The two are the same number today, because nothing has been accepted, and they stop being the same the day the gate at 6.7 accepts a version. Printing the constants would go on being right until then and then be wrong with nothing to say so.

**The planted-null tripwire is scoped to what a proposal rests on, and an abstention rests on nothing**
Settled at 6.5, closing the second thing asking a model found. The rule is that a proposal citing the planted null fails that pack version immediately, and the failure it was written against is a model that rummaged through the deciles and came back with the one signal that cannot matter (see: One meaningless signal is planted in the conditional tables).

**Fired on the name appearing anywhere, it catches that and three things that are its opposite**: an abstention naming the control in its reason, a mechanism sentence saying the control was checked and discarded, and a refutation condition that mentions it. **Demonstrated on the first real call**: shown the near-empty pack a model abstained, correctly and with an accurate reason, and put `day_of_month` in the gate field because the schema required one. Under the rule as written that correct abstention failed pack version 1. Reproduced at a second context size, and it is not a property of any transport, being what any model does when a required field has to be filled on an abstention.

**Two things close it and both are structural.** The tripwire's subject is the signals the proposal rests on, being the ones it cites as evidence and the ones the threshold it moves is replayed against, rather than the words of the document. And the change fields are conditional on not abstaining, in the type and in the store's own CHECK clauses, so an abstention has no gate field to fill and there is nowhere for the fault to happen (see: Abstention is a valid recorded proposal outcome).

**A stopgap fails no version whatever it cited**, which is not a third closure but the same one seen from the reader's side: a competent reader citing a meaningless signal indicts the pack and a weaker reader citing it indicts itself (see: A local stopgap seat is recorded, and it is excluded from the pack-version hit rate).

**The subscription seat is the Claude Code CLI in print mode, and the empty tool set is read back rather than asserted**
Settled at 6.5, because the transport the operator ruled for does not exist in this language. **The Agent SDK is a library for Python and TypeScript only**; the documented way to drive the same agent loop from another language is to run the CLI as a subprocess with `-p` and a structured output format. So the seat runs the CLI the SDK itself wraps, and what changes is not the transport but what an assertion about it can reach (see: The researcher transport is a configuration switch between subscription and API key, over a deliberately narrow interface).

**The empty tool set is harder here than the corpus assumed and ends up better held.** `--allowedTools` auto-approves and does not restrict, so passing it nothing would have left the full tool set in the model's context and merely unapproved; `--disallowedTools "*"` removes every tool from that context. That is an argument the seat builds and a test can read, and an argument is still only what the seat asked for. **So the session's own reported tool list is read back and the answer is refused if it is not empty**, and refused equally where no list was reported at all, because an unreadable guard is not a passed one. **That turns the 1.5 obligation from a test into a guard**: a tool arriving from configuration the seat does not control is refused at the ask rather than found later, and no test over the seat's own source could have seen one (see: The AI writes only to the proposal store).

**It asks from an empty directory, and that is load-bearing rather than tidy.** A print session without `--bare` loads CLAUDE.md, hooks, MCP servers and plugins discovered from its working directory, and this repository is full of all four. `--bare` is the documented fix and cannot be used, because it never reads the subscription login, which is the one thing this transport exists for. So the seat asks from a directory holding none of them and `--strict-mcp-config` closes the MCP half.

**And the cost figure the ruling quoted was wrong, which is recorded here because a decision is changed only by another decision.** The ruling said the API path is $4.13 a year and the whole published spread about eight dollars, taken from ARCHITECTURE's model budget on the day before the pack was measured. That table assumed 8,400 tokens in; the pack over a filled store is 23,545, so every per-run and per-year figure in it was understated by 2.8 times. **At this model the API path is $8.07 a year and the spread is about sixteen dollars.** It changes nothing: cost could not decide it at either figure, and that is the half worth keeping rather than the arithmetic. It is corrected in the document at 6.5 and named here so a later session reading the ruling does not carry the old number forward (see: A fixture expectation changes only with a recorded reason, and the report counts what changed).

**And a published statement now bounds the path, where the ruling recorded that none existed.** Anthropic's Agent SDK documentation states that third party developers may not offer claude.ai login or plan rate limits for their products without prior approval, and directs them to API key authentication. It is about redistribution to end users rather than about personal automation, so it does not reach a single-operator lab; what it does is name the boundary. **The subscription path stops being defensible the day this lab is offered to anyone else**, which is a condition the operator can see coming rather than a term that might move under them, and it sits beside a terms change and a cancellation as the third thing that can force the switch.
...
**A structural proposal has no representation and that is stated rather than discovered.** `SelectionRule` expresses a moved threshold over a fixed gate list, so a proposal that changes what a gate computes cannot be written down, cannot be replayed and is refused at the registry (see: Generation 1 opens with no version admissible in either family, and what reopens each is named).
...
**A surface a person opens is a screen, and the navigation names the way in apart from the research screens**
Taken at 7.20 on 2026-09-16, superseding the entry that fixed the count at six. **The principle and the pack page's standing carry over unchanged**: a surface a person opens is a screen whatever the count beside it says, the pack comparison page is one, and its subject is the set of pack versions rather than one night, one setup or one trade, which is why it hangs off no other screen. It carries the same withheld state the rest of the interface does.

**What moves is the count, and it moves because a flat list of eight would hide the distinction the two new surfaces exist to draw.** 7.20 adds a front page saying what the experiment is and where it has got to, and a day's page saying what the lab is watching and why. Both are screens by the rule above. Neither is the same kind of thing as the six: those are keyed on a night, a setup, a trade or the version set, and are read by somebody who already knows what the lab does, where these two are read by somebody who does not. One row of eight tabs asserts that all eight answer the same kind of question, and they do not.

**So the navigation is two named groups, and the count is derived rather than declared.** One group is the way in and one is the research screens, and the layout draws them apart. No number is written down anywhere: the shell test reconciles the navigation against the pages that exist in both directions, naming the one page that is reached for a stock rather than browsed to, and `architecture-conformance` reconciles those pages against the component catalogue. **That is the defect in the entry this supersedes**: it wrote a count into its own name, so the name went stale the moment a surface was added, which is the fault this corpus names whenever a figure is stated in prose and nothing derives it.
...
*Corrected on 2026-09-11, at 7.0(a), against the vendor.* "The vendor sells 120 days" read the width of one request as the depth of what is sold. The vendor sells one-minute history back to 2004, in requests of at most 120 days each; the measurement is recorded under **No execution variant is admitted in this generation, and the condition that would reopen it is named**. **No ruling changes here**, and the paragraph above is right in the one respect that matters: the forfeit is the lab's own rule and not a vendor limit, which is now true of both halves of the sentence rather than one.
...
**A slot that did not run is recorded the next night from its log, and a holiday is read from the index history**
Until 7.1 a slot that did not run left nothing: on 2026-09-07 thirty-one slots refused on the tree guard and `run_log` held no row for the date, so a refused night, a holiday, a paused lab and a machine that was off were one state in the store. `run_log` now gains a slot, the session a row is about, and one of four reasons, `refused-by-tree-guard`, `market-closed`, `paused-by-operator` and `fault`, and every slot the schedule fired that did not run gets a row saying which.

**The row is written the next night and not by the refusal, because the refusal cannot reach the writer.** The guard exits before the worker starts, and `RunLogger` is the table's one declared writer, attributed by the type that issues the statement. PowerShell writing the row would be a second writer; invoking a worker verb from the tree the guard has just refused, to record that the tree is untrusted, is circular and would itself refuse wherever that tree's migrations were ahead of the store. So the guard writes one structured line to the night's log, and `reconcile-night` reads it the next evening from a tree the guard passed. The older wording of the refusal is read as well, so the nights before the line existed can be reconciled by naming their date.

**Market closed is read from the index history, and the daily bars cannot answer it.** A night whose ingest never ran and a session the market never held look identical in `daily_bar`, because `daily-bars` asks the bulk endpoint for its own date and nothing else, so the night after a lost night never ingests it. `index-bars` refetches each tracker's whole history every night. So the reconciliation is the second verb of the `index` slot, and a weekday is market closed only where every tracker holds a later session and none holds that one. Where no later session is stored yet, nothing can tell a holiday from a lost night and the slot gets **no row**: a reason nothing establishes is not written, and the morning report already says the slot never ran. The plan put the reconciliation after the `bars` slot on the reading that the next night's daily ingest covers the session before; it does not, and the index slot is where that reading is true.

**A refusal is recorded as a refusal even on a holiday**, because it is what happened to the slot, and a fault is a slot whose starting line is in the log with nothing of it in `run_log`. The one slot that never writes a run entry, `snapshot-db`, is counted as unseeable when it fired, since on that slot a clean run looks the same, and gets a row only where the log gives a reason. **A pause is an act of the operator's**: a file named `paused` under the data root, read by the slot script before the tree guard, which writes the reason and exits 0 so thirty-two deliberate non-runs do not read as thirty-two failures (see: The nightly runs from its own checkout, updated once a night before the first slot).
...
Superseded on 2026-09-16, at 7.20, by **A surface a person opens is a screen, and the navigation names the way in apart from the research screens**. Its principle, that a surface a person opens is a screen whatever the count beside it says, is what the new entry applies rather than overturns, and the pack page's standing and its withheld state carry across unchanged. What moved is the count: the navigation now names two groups, being the way in and the research screens, because 7.20 adds a front page and a day's page that are read by somebody who does not yet know what the lab does.

**Generation 1's baseline is written clause by clause from SOURCES.md, and a stated qualifier makes a gate recorded rather than screening**
Three cases, and each has one treatment. Where the source gives a form and a number, generation 1 ships both, quoted. Where it gives a form and no number, it ships the form with the number marked as the author's and carried in the limits table, where the operator may replace it without reopening anything, and the once-only threshold adjustment carries over to generation 1 unspent (see: The once-only threshold adjustment is recorded unspent, and the baseline is frozen without it). Where the source gives a qualifier, the gate is recorded rather than screening: `moves-enough` is the instance, the figure being his own and his own words saying in the same breath that it "isn't a hard rule".

**What this cannot answer is a missing form or an uncomputable one**, and those are the operator's rather than the author's: the anchored average price's anchor, which he tunes to the candle the price has already respected and which cannot be computed at 18:30 the evening before, and the re-entry rule, for which no source gives a form at all. Both are carried as obligations due at the operator from 7.0.

Superseded on 2026-09-15, at 7.19, by **The baseline is written clause by clause from SOURCES.md, and a figure he states screens even where he qualifies it**. Its first two cases and its closing paragraph carry into that entry unchanged, and generation 1 was written under all three. What moved is the third: a qualifier no longer makes a gate recorded rather than screening, because on generation 1's first night the recorded daily range passed 18 of 24 long names below the figure he trades by, and generation 2 requires it.
...
*Corrected on 2026-09-03, at 5.2, against the live store.* The entry above rests on a premise about the running lab and the premise is false. **Measured**: `intraday_bar` holds **nought rows**, `intraday_fetch` nought, and `intraday-bars` appears nowhere in `run_log`, so the stage has never run once. `spread_snapshot` and `spread_pass` are empty on the same terms and `spreads` has never run either. The evidence store holds **four flagged nights**, 2026-08-27, 2026-08-28, 2026-08-31 and 2026-09-03, being **292 setup rows over 127 distinct names**, and not one minute of any of them was bought. Capture was to begin when 4.2 landed on 2026-09-01, and the three sessions that have closed since are 2026-09-01, 2026-09-02 and 2026-09-03. **The vendor sells no minute history at that resolution**, so a session not captured on its own night is gone at any price.

**What was wrong is "accumulate" and "grows at one night a night".** Nothing has accumulated. The screenable population is nought and has been nought throughout, because fifteen of the nightly map's thirty-two slots have never been registered with the scheduler and `intraday` is one of them, which `BUILD_PLAN.md` carries as its own obligation. A later session reading the paragraph as it stood would conclude minute bars had been building since 4.2 and would go looking for a population that does not exist.

**The refusal does not move and is strengthened.** Both routes by which an execution version earns its place were argued closed on the grounds that the screen has nothing to run over and the funnel produces no trade. The first is now closed harder than the entry claimed: it is not that the history is short, it is that there is none.

**What changes is the reopening condition, and it is now a start rather than a continuation.** The entry named the population continuing to grow. What it names now is that **capture begins**: the condition is `intraday` being scheduled and running, after which the screenable population starts at nought on that night and grows at one night a night from there. Until that happens no amount of waiting moves it. The funnel half is unchanged: a funnel that produces trades makes the forward route live.

**No ruling changes here.** The operator's answer of 2026-09-02 stands as given, and what is corrected is a measurement the entry asserted about the running lab rather than anything that was decided. It is the shape `CLAUDE.md` names when it says a green report is a statement about the build and never about the lab: the premise was about the running system, and nothing in the verification harness has the running system as its subject.

*Corrected on 2026-09-11, at 7.0(a), against the vendor.* **Measured**: one call through the lab's own `EodhdClient` on 2026-09-11 for `intraday/AAPL.US` at one-minute resolution over 2024-04-01, the first calibration session and about twenty-nine months back, returned **959 bars**, being 329 before the regular open, 390 inside it and 240 after the close, for 5 calls. The vendor's documentation lists one-minute US history from 2004, extended hours included, and gives 120 days as the most one request may span.

**What was wrong is the depth, in both paragraphs above.** 120 days is the width of one request and not the depth of what is sold. The horizon the corpus recorded, 2026-05-05 as at 2026-09-01, is exactly one request's width back from the day it was measured, so that probe measured its own window; an earlier probe of the same kind had stated its answer as a lower bound, and the qualifier was dropped between the two. **What a missed night actually forfeits is admissibility**: a minute observed after the night a name is flagged is invisible to that night's setups under the point-in-time rule, whatever the vendor would sell (see: The intraday fetch buys the twenty-seven session anchor window, and the count starts on the first night it runs at that width).

**No ruling changes here, and the refusal stands for this generation.** Its first ground is restated rather than removed: an execution rule cannot be screened over the evidence store because that store holds no minute from before capture began and none has been bought, which is a fact about this lab's purchases rather than about the market's history. History bought for a screen belongs to a separate research store, because a minute bought after its night cannot stand as that night's evidence.

Superseded on 2026-09-11, at 7.11, by **Generation 1 opens with no version admissible in either family, and what reopens each is named**. Its reasoning survives into generation 1 unchanged: both routes by which an execution version earns its place are closed, and a version nothing can score stays open for ever. What moved is the generation its heading names and the reopening condition, since the minute backfill of 7.7 gives an execution screen a history to run over for the first time and nothing yet runs one.
...
Superseded on 2026-09-11, at 7.11, by **Generation 1 opens with no version admissible in either family, and what reopens each is named**. The ruling carries into generation 1 as it was written, one threshold over the generation's own gate list; what the new entry adds is that generation 1 has no selection rule written down for a threshold to be named in, so nothing is admitted until one is.
...
Superseded on 2026-09-11, at 7.8, by **Order prices and the share count resolve at the entry minute**. What it got right survives: a size exists before the gate runs and the gate only reduces or blocks it. What moved is when the size exists. The plan now carries the entry rule rather than a price, so the size is resolved at the entry minute against the stop that minute gives, by the stage between the resolver and the gate, and the gate enforces the risk budget against that distance as a cap rather than asserting a figure written the evening before. The plan is still locked before the open; what it locks is the rule.
...
Superseded on 2026-09-11, at 7.8, by **Order prices and the share count resolve at the entry minute** and **The give-up gate is retired at selection and reborn as the entry-time ceiling**. Both of the two limits it kept away from the trigger now apply at the entry. Risk per trade is enforced by the gate against the stop the entry resolved, because the size is no longer the plan's; and the give-up distance is the entry ceiling, the tighter of half the daily range and 5%, applied to that stop by the stage that resolves it. The reason it gave for not re-applying the cap at trigger, that two implementations of one gate would disagree, is kept: generation 0's `exit-tight` still screens at detection until the switch night, and it is a selection clause, while the ceiling is a refusal of the entry, so the two answer different questions rather than one twice.
