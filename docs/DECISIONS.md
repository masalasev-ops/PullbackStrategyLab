...
**Which sessions the run covers is a separate question from how the detector reads them, and the two were confused until 2.11.** The averages are computed in memory, as above. The detectors are not: they read `indicator_daily` and `scan_hit` from the store, and both hold one session in the live store. So a run over the full stored history is not a run of what exists; it needs indicators, ladder grades and scan hits carried in memory, which is a second implementation of three stages whose only consumer is a table nothing downstream reads. The run at 2.11 therefore covers the golden fixture's seeded histories instead, replaying the nightly pipeline session by session into a scratch store so each stage writes its own rows and each detector reads them. That is the same code the lab runs at night, over real bars, on as many sessions as the fixture holds. The full-history run over the live universe is carried to 3.2 with its price stated, and this entry names it rather than leaving it to be rediscovered a third time.
...
The once stands even though the distribution narrowed. The run covers thirty names rather than two thousand, so the record names which distribution the adjustment was made against and states the rate per name per session beside the raw count: the 5 to 60 band is stated per side per night for the live universe, and applying it to a raw count from thirty names would set a threshold against a number that does not scale. The scaling is an assumption and is recorded as one. What it is not is a second calibration held in reserve, because a threshold adjusted twice against two populations is a threshold fitted to whichever gave the answer somebody preferred.
