...
### 2026-09-06 — BUILD_PLAN.md — cites A version whose moved gate cannot be judged from the frozen signals is refused at admission
Was:  three rows of the carried obligations table, raised at 3.5, 5.2 and 5.3 and all due at 6.1: `ForwardReturnFiller`'s quadratic re-walk, the two short-side gates a replay cannot judge, and the golden fixture's authored row recording verdicts from one geometry and freezing signals from another. And `None of the twenty-five rows above fall due at 4.17`.
Now:  the same three rows in a new section, "What the three due at 6.1 were", each carrying what happened to it; and the count reads twenty-three, the three leaving and one new row about the fixture's own prose arriving in the same pass.
Why:  all three were discharged at 6.1. The rows are moved rather than deleted, on the terms every other discharged pile in this file is: the count of obligations a checkpoint held is derived from the open rows plus the discharged ones, so a row that vanished would move the figure and a route counting live rows alone would empty as the work completed.

### 2026-09-06 — SCHEMA.md — cites A version whose moved gate cannot be judged from the frozen signals is refused at admission
...
### 2026-09-08 — BUILD_PLAN.md — cites Every phase ends in a generated phase report, not in a page somebody looks at
Was:  the carried obligations table holding sixteen rows and ten of them due at the operator, with "None of the sixteen rows above fall due at 4.17", the heading "### The ten that are the operator's, and what each one blocks", the reading "**none of the ten rows is a phase 5 question**", and the arrival narrative ending "and ten on 2026-09-08 when the five scheduled tasks were registered."
Now:  nineteen rows and thirteen at the operator, the same four sentences reading nineteen, thirteen, thirteen and ending "ten on 2026-09-08 when the five scheduled tasks were registered, and thirteen the same day when the measurement of the give-up gate's basis raised three at once."
Why:  three obligations were raised on 2026-09-08 by the measurement of what `exit-tight` rejects, all three due at the operator because each is a judgement no build session may take: the operational definition of "a nearby floor has just been established", whether a session the vendor prints with no trade in it is part of a pullback, and whether the calibration store is rebuilt so it can record an absence. `stated-counts` derives all four of these figures from the two tables, so the sentences move with the rows or the check fails, which is the registry doing what it exists for.

### 2026-09-08 — BUILD_PLAN.md — cites Every phase ends in a generated phase report, not in a page somebody looks at
Was:  "**Reading them as answered together or not at all turns nine rows into one wait**, and the last column is what says how wrong that reading is: eight of the nine block nothing in the build order today and the ninth blocks one count on one side."
Now:  the same sentence reading thirteen rows, with the split stated as eleven blocking nothing in the build order today, one blocking one count on one side, and one costing the reading of every funnel count on both sides.
Why:  the sentence was already stale at ten rows and nothing reads it, which is the shape the operator's own heading and the permit sentence were both caught in before they were registered. It is corrected here because this pass is the one adding rows to the table it describes, and leaving a knowingly wrong count beside three new rows is worse than the staleness it inherited. It is still unregistered: no claim in `stated-counts` derives it, so it can go stale again, and that is stated rather than left to be discovered.
### 2026-09-08 — ARCHITECTURE.html — cites Every phase ends in a generated phase report, not in a page somebody looks at
Was:  "Why the exit-tight check is the interesting one" closing on "The only way to get an exit tight enough is to enter during the day at a moment when a nearby floor has just been established. That is why this is an intraday strategy, and why a test built on daily closing prices alone would quietly be testing a different and worse strategy while appearing to test this one."
Now:  the SMCI worked example unchanged, then a paragraph keeping what the removed sentence was protecting, that this is the one gate deciding whether a setup is takeable at all rather than whether it is attractive; then a paragraph saying the premise is undefined, nothing in the corpus stating what establishes a floor or which minute the entry is taken at, and unimplemented, `intraday_bar` and `anchored_vwap` both holding nought rows, with the daily-bar answer's own figures; then a paragraph saying every funnel figure since 2.11 measures that mismatch, turning the old warning on the lab rather than withdrawing it, and naming the obligation due at the operator.
Why:  the finding recorded in `PROGRESS.md` on 2026-09-08 is that the defect is in this gate's basis rather than in its value, and the sentence removed here stated the basis as ground. Same grounds as the superseded 3.6 sentence corrected at 6.10: a reader opening only ARCHITECTURE would take a premise the record has already found against, and a spec is read to know the current state. What the sentence was protecting survives and is stated in its own paragraph, because the reason the gate is worth having does not depend on the premise being answerable today.

### 2026-09-08 — BUILD_PLAN.md — cites Every phase ends in a generated phase report, not in a page somebody looks at
Was:  the zero-range bar obligation and its operator row stating the population as 128,812 of 2,381,945 stored daily bars with 73,207 at a volume of nought over 743 dates, four calibration setups sitting on such a bar, and the enabling set being large while what is small is how often a name lands its pullback on one.
Now:  128,790 distinct bars on the latest observation of each with 73,186 at a volume of nought, of which 1,272 sit on a ticker that ever produced a setup and five sit inside a pullback window, reaching nine calibration setups and no live one, with one of the nine having its whole pullback on such a bar, and the shape gates refusing all 8,638 single-bar pullbacks so that no candidate can be ranked on one.
Why:  the operator asked for the count inside a pullback window rather than anywhere in the store before ruling, and the two differ by four orders of magnitude. The store figure alone overstates what is at risk, which is the fifth failure shape in the direction that inflates rather than the one that hides: a correct number computed over a population other than the one the ruling is about. The narrowing changes what is owed, from a guard against a name being ranked on a flattened pair to a judgement on whether four gates recording a value of nought is admissible evidence.
### 2026-09-08 — ARCHITECTURE.html — cites A gate handed an absent or degenerate quantity fails rather than passing
Was:  the gate lists followed by "A gate handed nothing fails, on both lists" and then the exit-tight note, with nothing between them.
Now:  two blocks between them. "And a nought is not evidence either", carrying the operator's ruling of 2026-09-08: a gate reading nought reads as the tightest possible pass rather than as empty, so a gate whose quantity is a distance records no value with the reason; `held-floor` and `no-reclaim` named as excluded with the counts that exclude them; the guard keying on range and never volume; and the reason it does not rest on `dip-shape`. And "Where this gate set came from", carrying the provenance figures.
Why:  the ruling is a property of every gate on both lists and the rule block above it is where a reader meets that class of statement, so a ruling recorded only in PROGRESS would be one a reader of the gate set never meets. The provenance sits there for the same reason and because the two are one fact from two sides: the gate set was authored and never traced, and the one gate that has been measured rests on a premise nothing defines.

### 2026-09-08 — BUILD_PLAN.md — cites Every phase ends in a generated phase report, not in a page somebody looks at
Was:  the carried obligations table holding nineteen rows with thirteen due at the operator, the zero-distance bar among them, and the operator's section reading thirteen in its heading, its phase-5 sentence, its arrival narrative and its one-wait sentence.
Now:  eighteen rows and twelve at the operator, a section recording what the zero-distance row was and the ruling that closed it, and the four sentences reading twelve.
Why:  the operator ruled it on 2026-09-08 and the code executing the ruling landed in the same commit, so the row is discharged rather than pending. A discharge moves a row into the record of what it was rather than deleting it, on the terms every other discharged pile in this file is kept, and `stated-counts` derives all four figures from the two tables so the sentences move with the row.
### 2026-09-08 — BUILD_PLAN.md — cites Every phase ends in a generated phase report, not in a page somebody looks at
Was:  the carried obligations table holding eighteen rows with twelve due at the operator, and the operator's section reading twelve in its heading, its phase-5 sentence, its arrival narrative and its one-wait sentence, the last of which said one row costs the reading of every funnel count on both sides.
Now:  nineteen rows and thirteen at the operator, the four sentences reading thirteen, and the one-wait sentence saying two rows carry that cost.
Why:  bringing the calibration walk current to 2026-09-04 put five nights into `calibration_setup` that the evidence store already held forward, which made the two populations comparable for the first time and measured the difference: 1.60 times the forward long rows and 2.69 times the short. The short ratio is the market-cap exemption and is already decided; the long ratio is a thrust ranked from stored windows where a forward night reads the vendor's 300-name scan, and nothing stated it. It cannot be repaired, because no vendor scan exists for a historical date at any price, so what is owed is a judgement about what the calibration figures are evidence of. `stated-counts` derives all four figures from the two tables, so the sentences move with the row.

### 2026-09-09 — CLAUDE.md — cites The corpus is nine documents, and the ninth records where the strategy came from
Was:  "Five specs and three records. A ninth document requires retiring one or writing down why not.", a lifecycle table of eight rows, and the closing citation naming the eight-document decision.
Now:  "Six specs and three records. A tenth document requires retiring one or writing down why not.", a ninth row for `SOURCES.md` marked spec, the citation repointed to the superseding decision, and a paragraph saying `SOURCES.md` is the ninth and nothing was retired for it.
Why:  the corpus had no place for what the strategy it models actually says. The gate set was authored from interviews nobody wrote down, which is what produced fourteen author's readings, and the reading could not be recorded anywhere that a check could read as a set. The ceiling moved by one and the rule that moves it is unchanged: retire one or write down why not, and the why not is in the decision.

### 2026-09-09 — CLAUDE.md — cites A clause states where it came from, and a sourced clause quotes rather than paraphrases
Was:  the Checks table without a `clause-provenance` row.
Now:  a row declaring it runs on every CI run and asserting that every clause SOURCES.md says rests on a source still names one, still resolves it against the source table, and still quotes it, with the verdict vocabulary closed and a source declared and never cited failing from the other side.
Why:  the roster lists every check that runs and the phase report enumerates checks by name, so a check that ran and was not declared here would be a property nobody wrote down. The property it holds is that a citation cannot be removed quietly, which is the same shape as a check narrowing its own scope: the document would still read as traced.

### 2026-09-09 — ARCHITECTURE.html — cites A clause states where it came from, and a sourced clause quotes rather than paraphrases
Was:  "Where this gate set came from" carrying the counts from the 2026-09-08 trace, being six of twenty forms and four of twenty thresholds resting on a stated source, fourteen an author's reading either way, and two clauses carrying no threshold; the six sourced forms and four sourced thresholds listed by name; and the closing paragraph saying the block sits beside the gate set. The twenty gate boxes carried no pointer.
Now:  four paragraphs carrying no counts, pointing at `SOURCES.md` for the trace and saying why the figures moved out: they were read off a table nothing derived, were correct on the day, and had no way of staying correct. What survives from the first trace is stated, being that the threshold column did not move and that a sourced clause is not thereby a faithful one. Each of the twenty gate boxes carries a pointer to its own clause in `SOURCES.md`.
Why:  the counts changed by nine on the form column when the material was read rather than the corpus, and a spec restating a figure another document derives is the same fact in two places. They are now derived by `stated-counts` from the clause tables. The per-box pointers exist because the question a reader meeting these twenty boxes has is which of them the strategy states and which this lab invented, and that question had no answer anywhere until the ninth document existed to hold it.

### 2026-09-09 — SCHEMA.md, BUILD_PLAN.md, ARCHITECTURE.html — cites The corpus is nine documents, and the ninth records where the strategy came from
Was:  five citations across the three files naming **The corpus is eight documents and a ninth requires retiring one**.
Now:  the same five sentences citing the superseding decision, unchanged in every other respect.
Why:  the decision moved to "Previously decided" and `no-superseded-citation` fails a spec citing one. The sentences are untouched otherwise, because none of them was about the ceiling's number: three are about `SCREENS.html` being retired, one about the Signals section staying a section, and one about where the sixth screen lives.

### 2026-09-11 — BUILD_PLAN.md — cites A clause states where it came from, and a sourced clause quotes rather than paraphrases
Was:  the 6.12 obligation's last sentence reading "A build session choosing the rule would be adding a fifteenth author's reading to a gate set in which fourteen of the twenty clause forms and fourteen of the twenty thresholds are already that".
Now:  "A build session choosing the rule would be adding one more unsourced rule to a gate set in which five of the twenty clause forms and fourteen of the twenty thresholds rest on no source".
Why:  the sentence restated the 2026-09-08 trace, which read the corpus because no document named a source. `SOURCES.md` read the material and moved the form column by nine, from fourteen with no source to five, and the threshold column by nothing. The branch that wrote `SOURCES.md` did not touch this row, so the figure shipped stale beside the document that derives it. It is now a `stated-counts` claim on each of its two numbers, derived from `SOURCES.md`'s clause rows, so the next move of either column moves this sentence or turns the suite red. The row's premise, that nothing defines the floor, is 7.0's to revisit and is not touched here.

### 2026-09-11 — ARCHITECTURE.html — cites No execution variant is admitted in this generation, and the condition that would reopen it is named
Was:  three sentences stating the vendor's minute history as bounded. The execution family's key block said minute bars exist only from the night capture begins "and the vendor sells no history to buy the gap back"; the replay-tier table's Execution row said they "exist for no night before it, and the vendor sells no minute history to buy the gap back"; and the morning-screen block said two slots lose something no later run can recover, "a quote has no history at all, and minute bars reach back a bounded number of days".
Now:  the key block says none of the vendor's earlier history, which it sells back to 2004, has been bought; the Execution row says minutes exist in the store for no night before capture, that the vendor sells that history back to 2004, that a minute bought after its night cannot stand as that night's evidence, and that history bought for a screen belongs to a separate research store; the morning-screen block says four slots lose something no later run can recover and names the minute bars' loss as admissibility.
Why:  7.0(a) read 959 one-minute bars for AAPL over 2024-04-01 through the lab's own client, about twenty-nine months back, and the vendor's documentation gives one-minute history from 2004 with 120 days as the most one request may span. The horizon the corpus had recorded was one request's width back from the day it was measured. The correction of record is appended to the decision entry this cites.

### 2026-09-11 — ARCHITECTURE.html — cites A version whose moved gate cannot be judged from the frozen signals is refused at admission
Was:  the replay note said selection is replayable for "all ten of the long side's movable thresholds and ten of the short side's twelve", named `averages-squeezing` and `reached-ceiling` as the two exceptions refused at admission, and said what would lift them is freezing the compared quantity; the replay-tier table's Selection row said nine of the long side's ten gates "and seven of the short side's ten are rebuilt".
Now:  all twelve of the short side's thresholds are replayable, the two gates are named as exceptions until 6.1 with the signals 6.1 froze for them, and the Selection row says nine and nine.
Why:  6.1 lifted both exceptions on 2026-09-06 by freezing `ema_gap_21_50_over_avg` and `ceiling_distance_ranges`, and the sentences were never revisited. `architecture-conformance` asserts nine and nine and printed a pass message saying so while the cell it validates said seven, because it compares code to code and reads only the row's first cell; the replay note sits in a block no check parses. The two gates are the short side's second and fourth most rejecting, so the stale sentences told a reader they could not be re-screened when they could.

### 2026-09-11 — ARCHITECTURE.html — cites A one-time backfill is outside the nightly ceiling, whether it buys daily history or minutes
Was:  "One-time backfill is about 7,000 calls over two days:"
Now:  "One-time backfill is about 7,000 calls, run outside the nightly ceiling because a one-time operation is not the evening's job", with the citation.
Why:  the two-day split was retired on 2026-08-25, when the ruling in `RUNBOOK.md` put a one-time operation outside the ceiling, and this sentence was not revisited. The ruling had no entry in `DECISIONS.md` until 7.0 gave it one and extended it to minutes.

### 2026-09-11 — ARCHITECTURE.html — cites Risk per trade stays at 0.75%, inside the strategy's own 0.5 to 1%
Was:  the authored-parameters row for risk per trade, basis "Midpoint of the 0.5 to 1% range the strategy is described with".
Now:  the same basis, saying it is the trader's own written sentence, with the citation. The value is unchanged.
Why:  the 2026-09-08 trace recorded the range as an author's paraphrase, and `SOURCES.md` found it in the trader's own guest post. A basis that quotes a source should say which, and the decision records that his later statements put the figure at 0.5% and why the value does not move.

### 2026-09-11 — BUILD_PLAN.md — cites No execution variant is admitted in this generation, and the condition that would reopen it is named
Was:  the 5.3 row saying minute bars exist only from the night 4.2 begins capturing them because "the vendor sells daily history and no purchase substitutes", and "**The gap is in history rather than in the code, so it does not expire and no later checkpoint closes it**"; and the 5.5 row saying the morning screen names "the two slots whose input cannot be bought back".
Now:  the 5.3 row says the store holds no minute from before that night and none of the vendor's earlier history has been bought, that the vendor sells it back to 2004, and that a purchase serves a separate research store because a minute bought after its night cannot stand as that night's evidence; its bold clause says the gap is in the evidence store and does not expire there. The 5.5 row says the slots whose loss no later run repairs.
Why:  the same measurement as the ARCHITECTURE entry above. The 5.3 row's conclusion that no later checkpoint closes the gap was true of the evidence store and false of any screen, and 7.7 is the checkpoint that gives the execution family's screen a history.

### 2026-09-11 — BUILD_PLAN.md — cites Generation 0 is retired as measuring the entry-level mismatch, and generation 1 registers only once its rule is whole
Was:  "Six phases." at the head of the plan; three sentences stating there is no phase 7, being the phase 6 out-of-scope note ("there is no phase 7 to defer to"), "**After 6.9 there is no phase 7.**", and the note after the 6.8 pile saying a session on `run_log` would settle the window question "and nothing schedules it, because after 6.9 there is no phase 7"; and no phase 7 section.
Now:  "Seven phases."; the three sentences record that there was to be no phase 7 and that 7.0 opened one, the third adding that 7.1 is what settles the window question; and a phase 7 section of thirteen rows, 7.0 to 7.12, placed before the carried obligations so both readers of the plan bound the sections where they bounded them before.
Why:  the operator instructed the realignment on 2026-09-10 and approved its plan on 2026-09-11. The corpus stated three times that the phase did not exist, which is a recorded position reversed, and it is written as a reversal rather than a heading added beside sentences that contradict it. `stated-counts` holds the phase count and phase 7's row count.

### 2026-09-11 — BUILD_PLAN.md — cites Order prices and the share count resolve at the entry minute
Was:  nineteen rows in the carried obligations table and thirteen at the operator, the 6.12 row asking what establishes a nearby floor and which minute the entry is taken at among them; the four sentences of the operator's section reading thirteen, and the one-wait sentence saying ten of the thirteen block nothing, one blocks one count on one side and two cost the reading of every funnel count.
Now:  twenty rows and fourteen at the operator. The nearby-floor row moves, verbatim, into a record section saying the decisions of 7.0 answered it; two rows arrive, being the anchored level's anchor rule and the re-entry rule, both due at the operator and both blocking nothing built. The operator's table loses and gains the same rows, and the four sentences read fourteen, with the split twelve, one and one.
Why:  `SOURCES.md` states the floor and the entry minute in the trader's own words, so the row's question has a citation for an answer and leaving it open beside decisions that answer it would be the corpus contradicting itself. What the source cannot answer, a missing form and an uncomputable one, is what the two new rows carry. The approved plan put the table at twenty-one and fifteen; it did not count the row this answers, and the difference is recorded in the 7.0 entry.

### 2026-09-11 — RUNBOOK.md — cites Every phase ends in a generated phase report, not in a page somebody looks at
Was:  "Rerun `actions` for that date, with `--with-dividends` if a dividend is what was missed."
Now:  "Rerun `actions` for that date. Dividends are requested by default and `--splits-only` is the one option that narrows it; any other option is refused by name rather than ignored."
Why:  `--with-dividends` exists in no source file. Following the row ran the stage and reported success, because the parse ignored every dashed argument but one, and it did the right thing only because dividends are the default. `ActionIngestor` now refuses an option it does not know, on the repair `recheck` was given after 3.13, and three tests exercise the refusal.

### 2026-09-11 — BUILD_PLAN.md — cites Every fixture expectation records how it was produced, and only the independently derived ones verify anything
...
### 2026-09-11 — SOURCES.md — cites Generation 1 trims 15% at 3R and again at 5R on both sides, and a short is held three sessions rather than trailed
Was:  no exits section; the "Repeated trims" row ending "The lab trims 15% once at 3R. The 15 and the 3R are his; the \"once\" is the lab's"
Now:  "Generation 1's exits", its prediction written before the trace, what the trace found, the two exit tables and four quoted sections; the row saying generation 0 trimmed once and the lab trims at 3R and 5R on both sides from 7.10
Why:  7.10 traces the exits before building them, and the row described the trim generation 0 ran.

### 2026-09-11 — ARCHITECTURE.html — cites Generation 1 trims 15% at 3R and again at 5R on both sides, and a short is held three sessions rather than trailed
Was:  the exit-rule row, long "Trail the 9-day average on the daily close, filling at the next open, or the fixed give-up point, whichever is reached first", short "Trim 15% of the planned position once at 3R, then exit if an hourly bar closes back above the 50-day average, or the fixed give-up point, whichever is reached first"; PositionManager's key "Longs trail and run; shorts trim and close. [...] the short rules are an intraday level and an hourly-close condition acted on inside the session", ending "The short's hourly close is measured against the average that stood before the session. [...]"; authored parameters "Short exit, trim fraction | 15% of the planned position, once, at 3R | [...] (see: The short trim is 15% of the planned position, once, at 3R)", "Short exit, the hourly grid | [...] Answered at 4.4.", and "Long exit, the other side of the comparison" opening "The short side had a stated exit trigger"
Now:  both cells trimming at 3R and again at 5R, the long trailing, the short closing at the next open once held three sessions; the key saying shorts are held a few sessions and close, and that a later trim is recorded beside the first; "Exit trims" on both sides with whose figures each are, a new "Short exit, hold limit" row, "The hourly grid" saying no exit reads it from 7.10, and the comparison row opening with its correction
Why:  The exits are generation 1's from 7.10, traced in SOURCES.md, and the once-only trim's decision is superseded.

### 2026-09-11 — SCHEMA.md — cites Generation 1 trims 15% at 3R and again at 5R on both sides, and a short is held three sessions rather than trailed
Was:  `trim_*` as "The short trim at 3R"; `exit_armed_*` naming "the short reclaim"; `trim_observed_at` "When the trim was observed. The third"; "Three stamps, because this is the one updated table in the phase. [...]"; "A position can end three ways from 4.8 [...] `exit_reason` is `give-up`, `trail` or `hourly-reclaim`"; `manage_run` "`closed_give_up`, `closed_trail`, `closed_reclaim`" and `trimmed` "shorts that reached 3R and were reduced"
Now:  the first trim on either side; the `further_*` columns and their stamp from migration 068; `exit_armed_*` naming the trail and the hold limit; four stamps and why the fourth is a column; `hold-limit` among the reasons; `closed_hold_limit`; `trimmed` counting every trim
Why:  Migration 068 records the trims after the first beside it, and the night counts the short side's new exit.

### 2026-09-11 — BUILD_PLAN.md — cites Generation 1 trims 15% at 3R and again at 5R on both sides, and a short is held three sessions rather than trailed
Was:  4.8's row citing "(see: The short trim is 15% of the planned position, once, at 3R)"; 7.10's row "[...] and where the source gives one side of an exit the other side is its mirror marked as the author's, so the short trail is a close above that average and generation 0's hourly close above the 50-day retires with it | [...] and a long and a short fixture position each exit at their expected trim and trail bars, never pooled |"
Now:  4.8's row citing the new decision, "which superseded the once-only trim at 7.10"; 7.10's row saying each side takes the form the trace records, a mirror only where the source gives one side, and the short side's own form where it gives one; its done condition naming the trail's bar on the long side and the hold limit's on the short
Why:  The trace contradicts the predicted short trail: he states he does not trail his shorts and holds them two to three days. The done condition is amended with the rule, and the PROGRESS entry says so.

### 2026-09-11 — SCHEMA.md — cites Generation 0 is retired as measuring the entry-level mismatch, and generation 1 registers only once its rule is whole
Was:  `setup` with no generation, and `calibration_setup`'s shape line ending "`degraded_because` and `corrected_check`."; no `setup_generation_zero`; `below_floor.generation` "0 until the switch night"; no GenerationOneForecast among the components that own no store; three citations of the two superseded variant entries
Now:  `setup.generation`, which `calibration_setup` does not carry, a calibration walk being generation 0's by construction; the `setup_generation_zero` section with its writers, disjoint by direction, and its one reader; `below_floor.generation` 1 from the switch night with that generation's floor; GenerationOneForecast owning none, with why; the citations naming **Generation 1 opens with no version admissible in either family, and what reopens each is named**
Why:  Migration 069 carries the switch night, and the two entries scoped to generation 0 are superseded at the switch.

### 2026-09-11 — ARCHITECTURE.html — cites Generation 1 opens with no version admissible in either family, and what reopens each is named
Was:  "The 61 components are listed by layer,", the P7 Builds cell ending "EntrySizer, EntryRuleMeasurement", and five citations of **No execution variant is admitted in this generation, and the condition that would reopen it is named** or **A version changes one threshold over the existing gate list, and structural change is out of scope for this generation**
Now:  "The 62 components are listed by layer,", the cell ending "EntryRuleMeasurement, GenerationOneForecast" with the component's catalogue row, and the five citations naming the entry that superseded both
Why:  GenerationOneForecast joins the catalogue, and a citation of a superseded entry outside a record fails `no-superseded-citation`.

### 2026-09-11 — BUILD_PLAN.md — cites Plans are resting orders and fills go in time order when the caps bind
Was:  the operator's heading "The fourteen that are the operator's"; the obligations row raised at 7.8 and due at 7.11, "Four stages key a night's plans by setup rather than by plan, and would throw once two versions plan one setup [...]"; "None of the twenty-one rows above fall due at 4.17"; 7.11's row "Before it, generation 1's gate set replayed over the frozen signals of the live nights since 7.4 states the expected flagged count [...]"; four citations of the two superseded variant entries
Now:  the row discharged into "What the plan-keying row was, and the change that closed it"; a row raised at 7.11 and due at the operator, whether the scoreboard reads across the switch, and the operator's heading and the sentence under it ("none of the fourteen rows is a phase 5 question") at fifteen; "None of the twenty-one rows above" again; 7.11's row naming GenerationOneForecast as what runs the replay; the four citations naming **Generation 1 opens with no version admissible in either family, and what reopens each is named**
Why:  7.11 keys the four stages by the plan with the test the row asked for, builds the forecast, supersedes the two entries scoped to generation 0, and finds the scoreboard reading `setup` whole across the switch, which is a question about the surface for the operator.

### 2026-09-11 — RUNBOOK.md — cites Generation 0 is retired as measuring the entry-level mismatch, and generation 1 registers only once its rule is whole
Was:  "The three that stay operator verbs, a fourth from 7.7 and a fifth from 7.9:" and "`slot-roster` excludes the five by name"; no switch procedure
Now:  a sixth operator verb, `forecast-generation-one`, and "excludes the six by name"; "The switch to generation 1, from 7.11", the five steps from migrating to reading the morning after
Why:  The forecast is an operator verb run on a store copy, and the switch is an irreversible act the operator takes.

### 2026-09-11 — BUILD_PLAN.md — cites Generation 0 is retired as measuring the entry-level mismatch, and generation 1 registers only once its rule is whole
Was:  the obligations row raised at 7.11 ending "Which of the three is right is a question about the surface rather than about the build, so it is the operator's. **Blocks nothing built**: it binds from the first switch night, which is the operator's act"; and the operator table's row "| 7.11 | **Whether the scoreboard reads across the switch**, and how | **Blocks nothing built.** It binds from the first switch night, which is the operator's act | Band 1 and the ceiling taken over rows two gate sets scored, with every count right and the population wrong |"
Now:  the same row, followed by a paragraph widening it at the 7.12 sign-off to the three weekly stages that read `setup` whole and store what they read, `ceiling`, `twin-pairs` and `admit-signals`, to `SetupReader.PopulationTo` as latent while the pack refuses, and to the two readers of the evening give-up distance, `ceiling` and `SetupCapper`, saying the readings are the operator's and the repairs a build session's, and binding for the weekly stages from the Saturday after the switch; the operator table's row saying the same in its three cells
Why:  The survey 7.11 ran stopped at the page. `CeilingCalculator` and `ScoredSetupReader` read `setup` with no generation, so the weekly stages store figures and rulings over two gate sets' rows from the first Saturday after the switch, which is the fifth shape in a stored result rather than on a surface. And from 7.8 the evening give-up distance is not the stop generation 1 takes, so the bound and the cap's rank carry generation 0's quantity as well as its population. One row widened rather than a second raised, because the question is the one 7.11 asked, being how each reader of `setup` treats the switch.

### 2026-09-11 — BUILD_PLAN.md — cites Every phase ends in a generated phase report, not in a page somebody looks at
Was:  "`fixtures/expectations.json` names thirteen frozen-only checkpoints under `frozenOnly`, of which nought still rest on an open obligation."
Now:  fourteen, the second half unchanged.
Why:  7.12 names its own permit, on the footing every phase sign-off since 2.12 has.

### 2026-09-11 — BUILD_PLAN.md — cites Every phase ends in a generated phase report, not in a page somebody looks at
Was:  under "The fifteen that are the operator's", "**Reading them as answered together or not at all turns fourteen rows into one wait**, and the last column is what says how wrong that reading is: twelve of the fourteen block nothing in the build order today, one blocks one count on one side, and one costs the reading of every funnel count on both sides. Answering any one of them is worth what that row says it is worth, and holding the other eight for it is a cost nobody chose. What the nine share is who can answer them and nothing else:"; and the count's history ending "and fourteen at 7.0, which answered the nearby-floor row with decisions citing `SOURCES.md` and carried the two questions the source leaves open.**"
Now:  "turns every row below into one wait", no row blocking a checkpoint in the build order today and one blocking only a checkpoint that is parked, "the others" and "What they share"; and the history carried on to fifteen at 7.11, with the 7.12 sign-off widening that row rather than adding one
Why:  The heading has read fifteen since 7.11 and the paragraph under it read fourteen, with figures of eight and nine from earlier states of the list beside it. None of the four is registered with `stated-counts`, so each went stale silently, which is the shape that check exists for. Each is now stated without a number the next row would stale again, and the history names the arrival 7.11 did not record.

### 2026-09-11 — RUNBOOK.md — cites Generation 1 trims 15% at 3R and again at 5R on both sides, and a short is held three sessions rather than trailed
Was:  the `manage` row "It runs the two rule sets and the give-up point together: the long trail on a daily close below the 9-day average, filling at the next open; the short trim of 15% of the planned size once at 3R, and the short exit on an hourly bar closing back above the 50-day average."; the `trades` row ending "A trimmed short's money is the trim's plus the close's, and its exit covered what the trim left"
Now:  generation 1's exits from 7.10, both sides trimming at 3R and again at 5R, the long trail, the short closed at the next open once held three sessions, and a short armed on the retired hourly close still filling under that name; "A trimmed position's money is its trims' plus the close's, on either side from 7.10, and its exit covered what the trims left"
Why:  7.10 changed the exits and edited ARCHITECTURE, SCHEMA and SOURCES with them, and the operator's own schedule table went on describing the exits generation 0 ran. `TradeJournal` reads the trimmed shares and money from `PositionReader`, which folds the later trims in on either side.

### 2026-09-11 — RUNBOOK.md — cites Generation 0 is retired as measuring the entry-level mismatch, and generation 1 registers only once its rule is whole
Was:  "The switch to generation 1, from 7.11" in five steps, step 1 reading "**Migrate the live store to 069**, on the terms of the section above: merge, the 17:00 update, then `tools/migrate.ps1` with `PullbackStrategyLab__DataRoot` set to `<repository>/data/live`." and step 2 "**Forecast on a store copy.** `forecast-generation-one <from> <to>` over the nights since 7.4 landed, with the data root set to the copy. Read the report: the flagged count per night, its call cost against the headroom the day's other stages left, and any names the fetch could not afford. A night over the headroom is a night whose later names go without minutes in ticker order, and that is decided before the switch rather than found on its first night."; the `backfill-minutes` row ending at its citation; and "The production checkout" saying nothing of what a clone needs carried into it
Now:  six steps. Step 1 says the 17:00 update task does not exist yet, so the update is `git pull --ff-only` by hand in the tree the tasks point at, with the migration in the same sitting between two slots. Step 2 runs the forecast over every night the store holds a snapshot for, reads each night's cost against the headroom on the row after it, and says a night over the headroom costs the next evening's first stages rather than the fetch's later names. A new step 3 names what binds from the switch night and asks for `ceiling` to be read before it. The `backfill-minutes` row says its calls are inside the vendor's allowance though outside the lab's count. "The production checkout" says a clone carries the store and the secrets file in on steps 1 to 6 of the move procedure
Why:  Found at the 7.12 sign-off reading the procedure as the operator will. The update step names a task RUNBOOK itself records as absent. The forecast runs the detector over stored figures and needs nothing 7.4 froze, and the production checkout has carried no 7.4 night. `IntradayFetcher` buys the names flagged the evening before the session it fetches, and it is the first spender in a quota day that opens at midnight UTC, so the report pairs a night with the wrong quota day and names the wrong casualty; the repair to the report is code and is recorded as a finding. And a clone derives its data root from its own location, so without the store and the secrets it loses its first night.

### 2026-09-11 — BUILD_PLAN.md — cites Long and short are never pooled into one figure
...
### 2026-09-12 — RUNBOOK.md — cites The lab keeps one store per purpose under one data root, and CI never opens the operator's
Was:  in first-time setup step 3, "because those two scripts name no root of their own and the 6.9 ruling has not been taken yet"; in "After a merge that carries a migration", "**Until the repair below lands that is also what this document prescribes**" and the ruling's closing "**Not taken at 6.9**, which ruled and could not repair without signing off code it had just written."
Now:  the scripts refuse when the variable is unset, naming the roots that exist and creating nothing; the section prescribes the same instruction without the "until" and records the 61 to 69 migration of 2026-09-12; and the ruling says it was taken at 7.14, with the test that holds it.
Why:  7.14 took the 6.9 ruling. A document telling the operator that a repair has not landed, when it has, sends them to compensate by hand for a fault that now refuses.

### 2026-09-12 — RUNBOOK.md — cites Generation 0 is retired as measuring the entry-level mismatch, and generation 1 registers only once its rule is whole
