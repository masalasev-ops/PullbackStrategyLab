...
| Store | Grain | Writer |
|---|---|---|
| `variant` | variant id | Insert VariantAdmitter / GenerationCloser, **disjoint by generation**: the admitter writes into the generation in force and the closer writes only the first row of the next one, its baseline (definition, target, min sample, **once**) · Update AcceptanceGate / GenerationCloser, **disjoint by the status written**: the gate writes `accepted` or `rejected` and the closer `unresolved` or `retired`, each only on a version still open and each only status and resolution date (see: Targets and minimum samples are written at creation and are immutable) |
...
| `status` | TEXT | `open`, `accepted`, `rejected`, `unresolved` or `retired`. The fourth is closed without an answer, which is what editing the baseline does, and it is not a rejection: a rejected version was measured and fell short, an unresolved one was never measured because what it was compared against stopped existing. **The fifth is the baseline's alone, from 7.6**, and it is what GenerationCloser writes on the baseline of the generation it closes: the baseline is the arm every comparison subtracts and is never itself accepted or rejected, and it is not unresolved either, being the thing whose going away makes the others so. The store holds `retired` to the baseline and holds a baseline to `open` or `retired` |
| `resolved_at` | TEXT NULL | When it was settled or closed, present exactly when the status is not `open`. AcceptanceGate and GenerationCloser write this and `status` and neither has a path to any column above |
