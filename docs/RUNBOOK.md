...
| 21:15 | `fills`, over the orders the gate placed and the positions carried in from earlier sessions. It spends no vendor call. Each resting order is priced at what it actually got: the trigger plus the whole spread the session captured, the wrong way, or the session's own opening price where the market gapped through the trigger overnight, which costs nothing on top because the gap is the crossing. A name the session quoted no usable two-sided book for is **not filled**, and the row says which blindness it was rather than the order disappearing: a fill charged nought is a free entry that clears every threshold written as a maximum. It also closes any position whose give-up point the session reached, at the same cost on the way out. **Until 4.8 a position closes only on its give-up point**, so a winner is held and occupies a slot the caps refuse the next morning's trigger on | 0 |
