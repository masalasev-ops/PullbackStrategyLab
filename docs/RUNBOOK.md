...
**Four stages the worker advertises are run by an operator and by no slot, and that is a decision per verb rather than an oversight.** `admit-signals` was the fourth and has the Saturday slot above from 7.4. The three that stay operator verbs, and a fourth from 7.7:
...
`slot-roster` excludes the four by name and holds this table to them, so a verb that gains a slot or loses its reason fails there rather than going quiet.
