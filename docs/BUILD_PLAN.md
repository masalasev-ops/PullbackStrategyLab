...
|---|---|---|
| 1.1 | **Done condition seven at 1.3, 1.4, 1.5 and 1.7: one expectation that is `DERIVED` or `CONFIRMED`, not only one carrying a tier.** The back-fill at 1.7 gave every expectation a tier, which is the half of the condition that copying figures out of a run can satisfy. All four checkpoints are still frozen-only, so what they hold detects change and verifies nothing. The cost is what has kept them open: their figures are whole-market counts over the captured symbol list and the captured bulk day, so a second implementation means reimplementing the security-type filter and both floors over the whole list rather than a formula over one ticker's window, and 1.7's four are facts about the fixture's own composition that the same job produces. 1.9 was the fifth and was closed at the 1.12 review, where the trackers' histories are per-symbol and derive from the captured responses for no vendor call. Until then `fixtures/expectations.json` names the four under `frozenOnly` and `fixture-replay` asserts that each names this row and that this row is still open, so the permission is read rather than remembered | 2.1 |
...
| 1.12 | **An out-of-scope coverage item names the checkpoint that ends it, the way an out-of-scope architecture claim already has to.** Today a claim carries a closing checkpoint and the report groups by it, so that count falls as checkpoints land; a coverage exemption carries prose. The rule does not transfer unmodified and that is why this is an obligation rather than a fix: `architecture-conformance` already names checkpoints, and two of `fixture-replay`'s exemptions close on a purchase rather than on a checkpoint, at 1,900 calls and about 130 MB for the whole-market screen and one per-ticker call for the floor's rejecting side. What the rule has to say about a condition that closes on a decision nobody has scheduled is the work. Moved from 2.6 to 2.2 at the 2.1 spec pass: 2.2 creates `setup` with three of its four declared writers unbuilt, which is what makes `writer-ownership` record a run of out-of-scope items carrying free prose, so 2.2 is where the rule first bites rather than the checkpoint after it | 2.2 |
| 1.12 | **The examined floor is compared per check and the property it guards is per scope.** `CheckCoverage.Report` sums every scope a check names and compares that one number against `fixtures/checks-baseline.json`. In five of the seventeen checks the sum is dominated by a size-of-corpus figure rather than by the property, so ordinary growth pays for a narrowing: at the phase 1 sign-off, `bar-append-only` cut to one bar table of three passed with five new source files added, and `path-casing` gutted to compare no paths at all passed with forty new string literals, both with `tools/ci.*` green and the phase report GREEN. The same sum gives false alarms from the other side, and that half fires first: removing two literals from one test file turned `path-casing` red. The repair is a floor per scope rather than per check, with the corpus-size scopes marked as context and never summed with the scope carrying the property, and it is code rather than a document, which is why it is carried here. The first checkpoint of phase 2 and the last one before phase 2 code starts growing the corpus | 2.1 |
