...
| 2.7 | **What market capitalisation a calibration run is entitled to read.** `tradable-shortable` needs a cap, and `SecurityReader` bounds the read on `sector_resolved_at` like every other point-in-time read, so a reconstructed 2024 session sees no cap: it was resolved in 2026 or not at all. The consequence is that a calibration run records no short rows, which makes the short half of the count distribution 2.11 exists to inspect empty. Three answers are available and none is obviously right: exempt the cap by name in calibration mode and record it as one more thing those rows carry beside survivorship bias, drop `tradable-shortable` from the calibration floor and count what the remaining nine produce, or accept an empty short distribution and set the short thresholds from the long side's. The choice belongs at 2.11, where the distribution is what a threshold is set against | 2.11 |
