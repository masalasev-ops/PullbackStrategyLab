...
| 2.6 | **Eight of the ten long checks are one-sided over the fixture, and the decision on the remedy is held for a human.** Measured at 2.6: only two setups are recorded, so a check with two results is one-sided unless those two disagree, and two do. The remedy the plan priced, a second as-of date at 33 per-ticker calls, does not close it: a second session over the same thirty names records one or two more setups and eight checks still have two or three results each. What would close it is a fixture with enough sessions or enough names to record tens of setups, which is a larger purchase than the one priced. The standing instruction declines more than two one-sided checks with a written paragraph and holds more than half; eight of ten is more than half | 2.12 |
