...
| 2.11 | One-time calibration, over the golden fixture's seeded histories rather than the full stored history. The nightly pipeline replayed session by session into a scratch store, so each detector reads the `indicator_daily` and `scan_hit` rows the stages that own them wrote for that session. The market-cap clause of `tradable-shortable` is exempted by name in calibration mode | Detector rows **into `calibration_setup`, never into `setup`** (see: The evidence store holds only setups flagged forward, never setups reconstructed from history), with the session count measured and recorded rather than asserted here. The count distribution is recorded per side twice, as a raw count and as a rate per name per session, because the 5 to 60 band is stated for a universe of two thousand names and this run covers thirty: a band applied to a raw count from thirty names is a threshold set against a number that does not scale. If the median rate scaled to the live universe falls outside 5 to 60 per side, the thresholds marked "phase 2 count check" are adjusted **once**, before phase 3, recorded as a dated event with before and after counts and naming which distribution it was set against and that the scaling is an assumption. Every calibration verdict records that the short side was measured against a nine-clause detector. A test asserts `setup` is still empty when calibration finishes |
...
|---|---|---|
| 1.6 | One `CONFIRMED` value per hand-picked ticker, checked against a charting platform's own readout. The derivation at 1.6 is a second implementation by the same author, which rules out a transcription error and not a shared misreading of a definition. The page that makes the comparison possible exists as of 1.10: `/chart/IESC` shows the three averages and the figures the lab stored, on the adjusted basis, in the two decimals a platform shows. What is left is a person reading a platform and writing the three figures down, which is the one step in this corpus a build session cannot do for itself. Moved from 1.12 to 2.11 at the 1.12 sign-off: it cannot be discharged by any build session, and holding a phase on an act no session can perform makes a due point permanent rather than pending. 2.11 is the first thing that reads these figures, since calibration is where a threshold is set against them | 2.11 |
| 2.7 | **What market capitalisation a calibration run is entitled to read.** `tradable-shortable` needs a cap, and `SecurityReader` bounds the read on `sector_resolved_at` like every other point-in-time read, so a reconstructed 2024 session sees no cap: it was resolved in 2026 or not at all. The consequence is that a calibration run records no short rows, which makes the short half of the count distribution 2.11 exists to inspect empty. **Answered before 2.11 and recorded in the calibration decision: the market-cap clause is exempted by name in calibration mode**, on the pattern `reached-ceiling`'s anchored clause already uses. Not an empty short side, because a threshold calibrated against an empty distribution is worse than none, and not dropping the gate, because that changes what the short side is without saying so. What is left is building it, and recording on every calibration verdict that the short distribution was measured against a nine-clause detector | 2.11 |
| 2.11 | **The calibration run over the full stored history, on the live universe.** 2.11 runs the nightly pipeline session by session over the golden fixture's seeded histories, which needs no path that does not exist: each stage writes its own rows into a scratch store and each detector reads them. The live store cannot be run that way. It holds one session of `indicator_daily` and `scan_hit`, and it may not receive reconstructed ones, because the engine computes for the members of a night's snapshot and there is no snapshot for a night the lab was not running. So the full-history run has a price, and the price is the thing to weigh rather than the run: a calibration mode that carries indicators, ladder grades and scan hits in memory, which is a second implementation of three stages whose only consumer is a table nothing downstream reads. Due at 3.2, where forward returns give a second reason to want a long reconstructed history and the same path would serve both | 3.2 |
| 2.9 | **The `CONFIRMED` gallery expectations.** The tier the corpus defines as what the chart page and the setup gallery produce has no entries at all, and no build session can make one: it is a person opening the gallery, reading a night, and writing down what they saw. The page is built and every property a session can assert about it is asserted, so what is left is the looking. Due at 2.12, where the gallery review is already part of the sign-off | 2.12 |
