...
/tools            ci.ps1  ci.sh  verify-phase  verify-phase.ps1  snapshot-db  migrate
                  ...
                  migrate.ps1  snapshot-db.ps1  the Windows wrappers for the two bash cells,
                  on the shape verify-phase.ps1 worked out. Added at 6.10
                  shell-provenance.ps1  shell-provenance.sh  what every entry point announces
                  itself through, and where the three wrappers' bash search lives. Sourced,
                  never executed, and carrying no shebang for that reason
...
| `coverage-reported` | every CI run | Every check the roster says runs is implemented, is invoked by `tools/ci.*`, states its own scope in numbers, and left a coverage record in the run the phase report reads. Every file in the suite that reads the shipped source belongs to a check, or declares in `fixtures/source-scans.json` what its scan asserts and which behavioural test exercises it, reconciled in both directions with the named test having to resolve |
...
| `check-completeness` | every CI run | Every setup row has a result recorded for every check defined at its date, with the check names read from ARCHITECTURE's own gate lists and reconciled in both directions. The date is real from 7.2: each row is held to the list `check_definition` says was in force on its night, every row of a side on one night records one set, and the latest night's set is the build's |
...
| `rendered-classes` | every CI run | Every class a rendered page carries has a rule in `lab.css`, or is declared as deliberately unstyled with why. The second surface check, and it reads what `surface-claims` cannot: that check reads a page for words and a colour is not a word, so an element that is present and invisible is outside it by construction. The reverse direction, a rule no page carries, is reported and never failed |
...
| `architecture-conformance` | every CI run | Every claim a table in ARCHITECTURE.html makes has a verdict: pass, fail, out of scope for this phase, or unexamined, and every table in the document is placed so none can go unread. And every claim whose cell states more than one clause records, clause by clause, whether its verdict reaches that clause, another named check does, the clause is the reason for a behaviour rather than a property, or nothing reaches it and what is missing. The clause text is compared verbatim, so a reworded cell loses its judgement rather than keeping it |
...
| `price-storage-form` | every CI run | No migration declares a price or money column `REAL`, and every ratio column a shipped statement selects is read back through the ratio crossing rather than the price one. The storage form is the half of the decimal rule that code review cannot see, and the crossing's name is the half whose values are identical either way |
...
| `slot-diagnostics` | every CI run | Every stage the slot script runs is invoked through the one function that keeps a native command's stderr, so a failing stage's message and the line saying the slot stopped both reach the night's log. And every refusal of the tree guard writes the line the next night's reconciliation reads before it exits |
| `slot-roster` | every CI run | The night's dispatcher, its own parameter set, the worker's advertised stages and RUNBOOK's schedule name the same slots and the same verbs, reconciled in every direction. And the four reasons a slot did not run for agree across Core, the store's constraint and the slot script, in both directions. And every stage the worker advertises that no slot runs is named, as an operator verb RUNBOOK lists or as a stage that is not the night's |
...
**The pass that plans a phase belongs to the phase it plans, at that phase's opening checkpoint.** `Phase 5 / 5.0` for the pass that writes phase 5's section, not `Phase 4 / 4.13` for the checkpoint the build happened to be standing on. The plan for a phase is that phase's first work and its first commit, and a log saying otherwise puts the largest single document change of a phase under the phase before it. Ruled by the operator on 2026-09-02, against both instances: the phase 4 plan went in as `Phase 3 / 3.15` and the phase 5 plan as `Phase 4 / 4.13`, the second inferred from the first. **This is distinct from the rule above it and does not disturb it**: work done during a phase that a later phase consumes stays with the phase that did it, which is what the 1.1 entry filed against a checkpoint five phases out says in its own words. What moves is a pass whose entire subject is the next phase.

**And it was unwritable rather than merely unwritten, which is why the mechanism moved with the rule.** A PROGRESS entry's heading was the whole record of a checkpoint having landed, so filing the plan under its own phase's opening checkpoint would have marked that checkpoint landed: every obligation due at it would have read as overdue, and the phase report would have moved a phase forward onto a checkpoint with no deliverable. An entry opening **"Not a checkpoint entry"** now says which checkpoint it belongs to without saying that checkpoint has landed. Twenty-four entries already opened that way and no checkpoint changes status by it, so the marker was there and nothing read it.
